...
def evaluate(m,panel,validation,root,label,checkpoint,batch,deadline,code_identity):
    ...
    binding=dict(label=label,checkpoint_sha256=digest,model_tensor_identity=before,code_identity=code_identity,panel_identity=panel['identity'],configuration_identity=identity(read_json(FROZEN/'EXECUTION.json')),batch_size=batch,mode='true_incremental_bf16_fp32_ce_fp64_nll',condition=m.condition)
    ...
    with preserve_rng('cuda'):
        ...
        for start in range(0,len(panel['sequences']),batch):
            ...
            assert time.time()<deadline-300,'evaluation budget cutoff'
            ...
            started=time.time();sums,state=ce(m,torch.from_numpy(x).cuda(),torch.from_numpy(y).cuda(),root,'EVALUATE',label=label,batch_start=start)
            ...
            progress(root,'BATCH_COMPLETE',label=label,completed=start+len(selected))
    ...
    atomic_json(root/(label+'_COMPLETE.json'),summary);return summary
