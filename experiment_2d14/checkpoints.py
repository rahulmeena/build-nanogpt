"""Complete R checkpoints, bounded local staging, atomic persistent publication."""
...
def validate(p,tiny=False):
    assert p['schema']=='2d14-R-complete-v1' and p['world_size']==1
    u=p['completed_updates'];assert p['accounting']==accounting(u) and p['next_lr_index']==u
    assert p.get('next_pass_count')==(pass_count(u+1) if u<1000 else None)
    ...
    if not tiny:assert (a['registered'],a['trainable'],a['frozen'])==(124697386,124697382,4)
...
def write(path,p,tiny=False):
    ...
    stage=os.environ.get('EXP2D14_LOCAL_STAGE')
    if stage:Path(stage).mkdir(parents=True,exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='2d14-checkpoint-',dir=stage) as directory:
        staged=Path(directory)/'checkpoint.pt'
        with staged.open('wb') as f:torch.save(p,f);f.flush();os.fsync(f.fileno())
        reopened=torch.load(staged,map_location='cpu',weights_only=False,mmap=True)
        audit=validate(reopened,tiny);digest=sha256(staged)
        with staged.open('rb') as src,tmp.open('wb') as dst:
            shutil.copyfileobj(src,dst,8<<20);dst.flush();os.fsync(dst.fileno())
        assert sha256(tmp)==digest
        os.replace(tmp,path)
