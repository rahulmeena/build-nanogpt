#!/usr/bin/env python3
"""Experiment 2D2D: B2 W32 plus the older B11-to-B2 recurrent K/V bank.
...
trained B12-to-B1 path; gives B2 native W32 context; and adds exactly one
scalar gate for an attached B11-to-B2 bank restricted to lags 32..1023.
"""
...
from experiment_2d2d_core import (  # noqa: E402
    BANK_MODES,
    B2_LOCAL_WINDOW,
...
EXPERIMENT = "2D2D"
PROTOCOL = "exp2d2d_b2_w32_b11_recurrent_992_v1"
BRANCH = "experiment-2d2d-b2-w32-b11-recurrent-992"
...
CONFIG_PATH = REPO_ROOT / "configs" / "exp2d2d_b2_w32_b11_recurrent_992.json"
OUTPUT_NAME = "experiment_2d2d_b2_w32_b11_recurrent_992"
CHECKPOINT_SCHEMA = "exp2d2d_b2_w32_b11_recurrent_992_checkpoint_v1"
...
IMPLEMENTATION_FILES = (
    "configs/exp2d2d_b2_w32_b11_recurrent_992.json",
    ...
    "train_gpt2.py",
    "tests/test_experiment_2d2d_core.py",
    "tests/test_experiment_2d2d_driver.py",
)
REQUIRED_ARTIFACTS = (
    "EXPERIMENT_2D2D_FINAL_REPORT.md",
    "FINAL_AUDIT.json",
    "result_summary.json",
    "source_manifest.json",
    "parameter_manifest.json",
    "architecture_manifest.json",
    "2d2c_matched_reference_manifest.json",
    "matched_2d2c_data_replay_audit.json",
    "semantic_diff_audit.json",
    "batch_manifest.json",
    "preflight_audit.json",
    "initial_b2_w32_compression.json",
...
def gradient_group_report(model):
    """Report base and both scalar-gate gradients without cross-contamination."""

    base = [
        parameter.grad
        for name, parameter in model.named_parameters()
        if name not in {"g_rec", "g_rec_b2"} and parameter.grad is not None
    ]
...
def require_git(clean: bool = True) -> None:
    ...
    if git_output("rev-parse", FROZEN_2D2C_TAG + "^{commit}") != FROZEN_2D2C_COMMIT:
        raise SystemExit("frozen 2D2C tag mismatch")
    subprocess.check_call(
        ["git", "merge-base", "--is-ancestor", FROZEN_COMMIT, "HEAD"], cwd=REPO_ROOT
    )
    if git_output("merge-base", "HEAD", FROZEN_2D2C_COMMIT) != FROZEN_COMMIT:
        raise SystemExit("2D2D and 2D2C do not share exact 2D2B sibling base")
    if subprocess.run(
        ["git", "merge-base", "--is-ancestor", FROZEN_2D2C_COMMIT, "HEAD"],
        cwd=REPO_ROOT,
        check=False,
    ).returncode == 0:
        raise SystemExit("2D2D must not descend from 2D2C")
    if clean and git_output("status", "--porcelain"):
        raise SystemExit("result-bearing 2D2D worktree must be clean")
...
def require_config() -> dict:
    ...
    checks = {
        ...
        "frozen_2d2c": config.get("frozen_2d2c_tag") == FROZEN_2D2C_TAG
        and config.get("frozen_2d2c_commit") == FROZEN_2D2C_COMMIT
        and config.get("frozen_2d2c_checkpoint_sha256")
        == FROZEN_2D2C_CHECKPOINT_SHA256,
        ...
        "matched_b2_gate_optimizer": config["training"]["optimizer"][
            "new_b2_gate_group_matches_frozen_2d2c"
        ]
        is True
        and config["training"]["optimizer"]["new_b2_gate_lr"] == GATE_LR
    ...
    if not all(checks.values()):
        raise SystemExit(f"2D2D preregistration mismatch: {checks}")
    return config
...
def workspace_mount_audit(output_dir, run_root, supplied_identity) -> dict:
    ...
    if output != expected_output:
        raise SystemExit(f"2D2D result directory must be exactly {expected_output}")
    if not str(output).startswith("/workspace/") or not str(run).startswith("/workspace/"):
        raise SystemExit("2D2D results and checkpoints must live under /workspace")
...
def authenticated_stop_audit(args) -> dict:
    ...
    checks = {
        "schema": payload.get("schema") == "exp2d2d_runpod_stop_capability_v1",
...
def parameter_manifest(model, source_payload) -> dict:
    ...
    report = {
        ...
        "2d2d_named_parameter_inventory": inventory,
        ...
        "2d2d_total_parameters": sum(row["numel"] for row in inventory),
    ...
    report["checks"] = {
        ...
        "target_total": report["2d2d_total_parameters"] == TOTAL_PARAMETERS,
...
def matched_2d2c_reference_manifest() -> dict:
    return {
        "schema": "exp2d2d_frozen_2d2c_matched_reference_v1",
        ...
        **copy.deepcopy(FROZEN_2D2C_REFERENCE),
        ...
        "sole_scientific_difference": (
            ...
            "2D2D local W32 plus recurrent lags 32..1023"
        ),
    }
...
def frozen_2d2c_optimizer_audit(current_report) -> dict:
    ...
    return {
        ...
        "current_2d2d_optimizer_report": current_report,
        "checks": checks,
        "passed": all(checks.values()),
    }
...
def initialize_matched_data_replay_audit(source_audit, source_stream, scientific_stream):
    return {
        "schema": "exp2d2d_matched_2d2c_data_replay_audit_v1",
        ...
        "unavailable_comparisons": (
            ...
            "is compared in memory without creating extra 2D2D checkpoints."
        ),
...
def record_matched_replay_cursor(
    ...
    audit["checkpoint_cursor_comparisons"][str(update)] = {
        ...
        "observed_2d2d_next_global_batch_sha256": observed_batch,
        "expected_2d2c_next_global_batch_stream_sha256": reference[
            "next_stream_sha256"
        ],
        "observed_2d2d_next_global_batch_stream_sha256": observed_stream,
...
def evaluate_parallel(
    ...
    for batch_index in range(batches):
        ...
        for name in names:
            ...
            del output, tensor
            torch.cuda.empty_cache()
        print(f"2D2D validation batch={batch_index + 1:02d}/{batches}", flush=True)
        del x, y, cpu_x, cpu_y
        torch.cuda.empty_cache()
...
def probe_microbatch(model, optimizer, shards, device, candidates=(32, 16, 8, 4, 2)):
    ...
    for candidate in candidates:
        ...
        try:
            ...
            gc.collect()
            torch.cuda.empty_cache()
    raise SystemExit(f"no safe 2D2D microbatch: {attempts}")
...
def run_preflight(args):
    ...
    if not optimizer_match["passed"]:
        raise SystemExit(
            f"2D2D optimizer differs from frozen matched 2D2C: {optimizer_match['checks']}"
        )
    ...
    durable_json(
        ...
        {
            "schema": "exp2d2d_storage_cleanup_manifest_v1",
            "preflight_cleanup": {
                "action_provenance": (
    ...
    if (
        ...
        raise SystemExit(
            "2D2D did not preserve frozen one-GPU 2D2C microbatch/reduction geometry"
        )
    ...
    if not preflight["science_passed"]:
        raise SystemExit(f"2D2D preflight failed: {science_checks}")
    print("EXPERIMENT_2D2D_PREFLIGHT_PASS", flush=True)
    return preflight
...
def checkpoint_payload(model, optimizer, loader, training_state, metadata, accumulation):
    return {
        ...
        "completed_2d2d_updates": training_state["completed_2d2d_updates"],
        "processed_2d2d_targets": training_state["processed_2d2d_targets"],
...
def strict_reopen_checkpoint(
    ...
    checks = {
        ...
        "updates": reopened.get("completed_2d2d_updates")
        == training_state["completed_2d2d_updates"],
        "additional_targets": reopened.get("processed_2d2d_targets")
        == training_state["processed_2d2d_targets"],
    ...
    verification = {
        ...
        "bytes": path.stat().st_size,
        "completed_2d2d_updates": reopened["completed_2d2d_updates"],
...
def run_smoke(args):
    ...
    with torch.no_grad():
        ...
        cache = model.incremental_logits(
            cache_x[:, :64].to(device), control="both_real", bank_mode="full"
        )["cache_audit"]
    state = {
        "completed_2d2d_updates": 3,
        "processed_2d2d_targets": 3 * smoke_batch * T,
    ...
    if not audit["passed"]:
        raise SystemExit(f"2D2D smoke failed: {checks}")
    ...
    durable_json(output / "smoke_audit.json", audit)
    print("EXPERIMENT_2D2D_SMOKE_PASS", flush=True)
    return audit
...
def initialize_runtime(args):
    ...
    if args.resume:
        ...
        if state["completed_2d2d_updates"] != FORCED_RESTART_UPDATE:
        ...
        restart = {
            ...
            "completed_2d2d_updates": state["completed_2d2d_updates"],
        ...
        state = {
            "completed_2d2d_updates": 0,
            "processed_2d2d_targets": 0,
        ...
        saved_pid = None
    if state["completed_2d2d_updates"] == 0:
...
def train_one_update(runtime, update):
    ...
    elapsed = time.monotonic() - start
    runtime.training_state["completed_2d2d_updates"] = update
    runtime.training_state["processed_2d2d_targets"] = update * GLOBAL_TARGETS
...
def milestone_diagnostics(runtime, update, val_path):
    ...
    validation["matched_2d2c_comparison"] = {
        "gain_W32_minus_gain_W2": validation["b2_recurrent_gain"] - reference_gain,
        "sequence_gap_W32_minus_sequence_gap_W2": validation["b2_sequence_gap"]
...
def reconcile_uncheckpointed_artifacts(output, completed):
    ...
    output = Path(output)
    audit = {
        "schema": "exp2d2d_resume_reconciliation_v1",
...
def run_train(args):
    ...
    if int(args.end_update) not in (FORCED_RESTART_UPDATE, MAX_UPDATES):
        raise SystemExit("2D2D train segments must end at local update 96 or 191")
    runtime = initialize_runtime(args)
    completed = int(runtime.training_state["completed_2d2d_updates"])
    ...
    for update in range(completed + 1, int(args.end_update) + 1):
        ...
        print(
            f"2D2D update={update:03d}/{MAX_UPDATES} "
        ...
        if str(update) in FROZEN_2D2C_REFERENCE["available_cursor_hashes"]:
            record_matched_replay_cursor(
                runtime.output,
                update,
    ...
    if int(args.end_update) == FORCED_RESTART_UPDATE:
        print("EXPERIMENT_2D2D_UPDATE_96_RESTART_REQUIRED", flush=True)
    else:
        durable_json(
            runtime.output / "training_complete.json",
            {
                "completed_2d2d_updates": MAX_UPDATES,
                "processed_2d2d_targets": ADDITIONAL_TARGETS,
                "cumulative_2d2_targets": CUMULATIVE_TARGETS,
                "checkpoint": runtime.training_state["last_checkpoint"],
                "timestamp": time.time(),
            },
        )
        print("EXPERIMENT_2D2D_TRAINING_COMPLETE", flush=True)
    return segment
...
def evaluate_incremental(model, val_path, batches=INCREMENTAL_BATCHES) -> dict:
    ...
    for batch_index in range(batches):
        ...
        print(f"2D2D incremental batch={batch_index + 1:02d}/{batches}", flush=True)
        del x, y, cpu_x, cpu_y
        torch.cuda.empty_cache()
    elapsed = time.monotonic() - start
    controls = {}
...
def parallel_incremental_equivalence(model, val_path, length=64, batch=2):
    if length < 64:
        raise ValueError("2D2D equivalence requires length >=64 to exercise B2 W32 eviction")
...
def load_final_model(args, device):
    ...
    if payload.get("completed_2d2d_updates") != MAX_UPDATES:
    ...
    audit = {
        ...
        "completed_2d2d_updates": payload["completed_2d2d_updates"],
        "processed_2d2d_targets": payload["processed_2d2d_targets"],
...
def classify_result(incremental, stable=True, integrity=True) -> str:
    if not integrity:
        return "EXPERIMENT 2D2D INVALID"
    if not stable:
...
def choose_recommendation(classification, initial_damage) -> str:
    if classification == "EXPERIMENT 2D2D INVALID":
        return "FIX 2D2D INTEGRITY"
...
def make_plots(output, milestones, training, incremental, attention, temporal, performance):
    ...
    def line_plot(filename, series, ylabel):
        ...
        ax.set_xlabel("Additional 2D2D training targets")
    ...
    ax.set(xlabel="Additional 2D2D training targets", ylabel="tanh(g_rec_B1)")
    ax.grid(alpha=0.25)
    ax.legend()
    fig.tight_layout()
    ...
    ax.plot(tx, [row["tanh_g_rec_b2"] for row in training], label="2D2D W32")
    ax.plot(
        milestone_targets,
...
def render_report(summary, audit, questions):
    ...
    lines = [
        f"EXPERIMENT 2D2D PRIMARY CLASSIFICATION:\n{summary['primary_classification']}",
...
def build_integrity_checks(
    ...
    checks = {
        ...
        "191 additional optimizer updates": final_model_audit["completed_2d2d_updates"] == 191,
        "100,139,008 additional targets": final_model_audit["processed_2d2d_targets"] == ADDITIONAL_TARGETS,
...
def run_finalize(args):
    ...
    if training_complete["completed_2d2d_updates"] != MAX_UPDATES:
    ...
    durable_text(output / "EXPERIMENT_2D2D_FINAL_REPORT.md", report)
    durable_text(
    ...
    if not scientific_integrity or not inventory["passed"]:
        raise SystemExit(
            f"2D2D finalize integrity failed: scientific={scientific_integrity} inventory={inventory['passed']}"
        )
    print("EXPERIMENT_2D2D_FINALIZED_PENDING_GIT_SEAL", flush=True)
    return summary
...
def run_seal_report(args):
    ...
    print("EXPERIMENT_2D2D_REPORT_SEALED", flush=True)
...
def run_attest_seal(args):
    ...
    print("EXPERIMENT_2D2D_SEAL_ATTESTED", flush=True)
