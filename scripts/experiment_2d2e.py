#!/usr/bin/env python3
"""Experiment 2D2E: B3 W64 plus the older B10-to-B3 recurrent K/V bank.
...
trained B12-to-B1 and B11-to-B2 paths; changes only B3 from W1024 to W64; and
adds exactly one scalar gate for B10-to-B3 recurrence at lags 64..1023.
"""
...
from experiment_2d2e_core import (  # noqa: E402
    BANK_MODES,
    B2_LOCAL_WINDOW,
...
EXPERIMENT = "2D2E"
PROTOCOL = "exp2d2e_b3_w64_b10_recurrent_960_v1"
BRANCH = "experiment-2d2e-b3-w64-b10-recurrent-960"
FROZEN_TAG = "experiment-2d2d-b2-w32-b11-recurrent-992-final"
FROZEN_COMMIT = "a9300a9800f2e2c46f3892cff52b0a4a2a547d11"
CONFIG_PATH = REPO_ROOT / "configs" / "exp2d2e_b3_w64_b10_recurrent_960.json"
OUTPUT_NAME = "experiment_2d2e_b3_w64_b10_recurrent_960"
...
IMPLEMENTATION_FILES = (
    "configs/exp2d2e_b3_w64_b10_recurrent_960.json",
    ...
    "train_gpt2.py",
    "tests/test_experiment_2d2e_core.py",
    "tests/test_experiment_2d2e_driver.py",
)
REQUIRED_ARTIFACTS = (
...
REQUIRED_PLOTS = tuple(f"P{index}_{name}.png" for index, name in enumerate((
    ...
    "b1_b2_b3_temporal_distance_comparison",
    "parallel_vs_true_incremental_b3_gain",
    "b1_b2_b3_marginal_recurrent_gains",
...
def gradient_group_report(model):
    """Report base and all three scalar-gate gradients independently."""

    base = [
        parameter.grad
        for name, parameter in model.named_parameters()
        if name not in {"g_rec", "g_rec_b2", "g_rec_b3"} and parameter.grad is not None
    ]
...
def require_git(clean: bool = True) -> None:
    ...
    if clean and git_output("status", "--porcelain"):
        raise SystemExit("result-bearing 2D2E worktree must be clean")
...
def require_config() -> dict:
    ...
    checks = {
        ...
        "hardware": config["hardware"]["pod_id"] == "7kk5yyti00rnrp"
        and config["hardware"]["pod_name"] == "grand_amber_catshark"
        and config["hardware"]["gpu_count"] == 1,
    }
    if not all(checks.values()):
        raise SystemExit(f"2D2E preregistration mismatch: {checks}")
    return config
...
def workspace_mount_audit(output_dir, run_root, supplied_identity) -> dict:
    ...
    if output != expected_output:
        raise SystemExit(f"2D2E result directory must be exactly {expected_output}")
    if not str(output).startswith("/workspace/") or not str(run).startswith("/workspace/"):
        raise SystemExit("2D2E results and checkpoints must live under /workspace")
...
def authenticated_stop_audit(args) -> dict:
    ...
    checks = {
        "schema": payload.get("schema") == "exp2d2e_runpod_stop_capability_v1",
...
def configure_optimizer(model, device_type="cuda", include_new_gate=True):
    """Recreate 2D2D groups, then optionally add only the fresh B3 gate."""

    base_decay, base_nodecay = [], []
    for name, parameter in model.named_parameters():
        if not parameter.requires_grad or name in {"g_rec", "g_rec_b2", "g_rec_b3"}:
            continue
...
def parameter_manifest(model, source_payload) -> dict:
    ...
    report = {
        ...
        "2d2e_named_parameter_inventory": inventory,
        ...
        "2d2e_total_parameters": sum(row["numel"] for row in inventory),
    ...
    report["checks"] = {
        ...
        "target_total": report["2d2e_total_parameters"] == TOTAL_PARAMETERS,
...
def architecture_manifest() -> dict:
    return {
        ...
        "parallel_source_tensor_shapes": {"h10": "[B,T,C]", "h11": "[B,T,C]", "h12": "[B,T,C]"},
...
def checkpoint_payload(model, optimizer, loader, training_state, metadata, accumulation):
    return {
        ...
        "completed_2d2e_updates": training_state["completed_2d2e_updates"],
        "processed_2d2e_targets": training_state["processed_2d2e_targets"],
...
def strict_reopen_checkpoint(
    ...
    checks = {
        ...
        "updates": reopened.get("completed_2d2e_updates")
        == training_state["completed_2d2e_updates"],
        "additional_targets": reopened.get("processed_2d2e_targets")
        == training_state["processed_2d2e_targets"],
    ...
    verification = {
        ...
        "bytes": path.stat().st_size,
        "completed_2d2e_updates": reopened["completed_2d2e_updates"],
...
def initialize_runtime(args):
    ...
    if args.resume:
        ...
        if state["completed_2d2e_updates"] != FORCED_RESTART_UPDATE:
        ...
        restart = {
            ...
            "completed_2d2e_updates": state["completed_2d2e_updates"],
        ...
        state = {
            "completed_2d2e_updates": 0,
            "processed_2d2e_targets": 0,
        ...
        saved_pid = None
    if state["completed_2d2e_updates"] == 0:
...
def train_one_update(runtime, update):
    ...
    for micro_index in range(runtime.accumulation):
        ...
        del x, y, cpu_x, cpu_y, results, source_h10, source_h11, source_h12, weighted, scaled, current
    if not gradients_finite(model):
        raise SystemExit("nonfinite gradients")
    groups = gradient_group_report(model)
    if not all(
        groups[name]["finite"] and groups[name]["nonzero"]
        for name in ("base", "gate", "b2_gate", "b3_gate")
    ):
    ...
    elapsed = time.monotonic() - start
    runtime.training_state["completed_2d2e_updates"] = update
    runtime.training_state["processed_2d2e_targets"] = update * GLOBAL_TARGETS
...
def reconcile_uncheckpointed_artifacts(output, completed):
    ...
    output = Path(output)
    audit = {
        "schema": "exp2d2e_resume_reconciliation_v1",
...
def run_train(args):
    ...
    if int(args.end_update) not in (FORCED_RESTART_UPDATE, MAX_UPDATES):
        raise SystemExit("2D2E train segments must end at local update 96 or 191")
    runtime = initialize_runtime(args)
    completed = int(runtime.training_state["completed_2d2e_updates"])
    ...
    for update in range(completed + 1, int(args.end_update) + 1):
        ...
        print(
            f"2D2E update={update:03d}/{MAX_UPDATES} "
    ...
    if int(args.end_update) == FORCED_RESTART_UPDATE:
        print("EXPERIMENT_2D2E_UPDATE_96_RESTART_REQUIRED", flush=True)
    else:
        durable_json(
            runtime.output / "training_complete.json",
            {
                "completed_2d2e_updates": MAX_UPDATES,
                "processed_2d2e_targets": ADDITIONAL_TARGETS,
                "cumulative_2d2_targets": CUMULATIVE_TARGETS,
                "checkpoint": runtime.training_state["last_checkpoint"],
                "timestamp": time.time(),
            },
        )
        print("EXPERIMENT_2D2E_TRAINING_COMPLETE", flush=True)
    return segment
...
def load_final_model(args, device):
    ...
    if payload.get("completed_2d2e_updates") != MAX_UPDATES:
    ...
    audit = {
        ...
        "completed_2d2e_updates": payload["completed_2d2e_updates"],
        "processed_2d2e_targets": payload["processed_2d2e_targets"],
...
def semantic_diff_audit(model, source_payload) -> dict:
    ...
    report["passed"] = all(
        report[key]
        for key in ("source_state_dict_keys_preserved", "b1_gate_resumed", "b2_gate_resumed", "b3_gate_zero")
    )
    if not report["passed"]:
...
def evaluate_parallel(model, val_path, batches=VALIDATION_BATCHES, combined_controls=False) -> dict:
    ...
    if combined_controls:
        names.extend((
            "all_shuffled", "b2_off_b3_real", "b1_off_b2_real_b3_real",
            "b1_real_b2_off_b3_real", "b1_real_b2_real_b3_off",
            "b1_real_b2_off_b3_off",
            "b1_real_b2_shuffled_b3_real", "b1_real_b2_real_b3_shuffled",
        ))
    accumulators = {
    ...
    for batch_index in range(int(batches)):
        ...
        for name in names:
            kwargs = {}
            if name in {"b3_off", "b1_real_b2_real_b3_off", "b1_real_b2_off_b3_off"}:
                kwargs["b3_gate_override"] = 0.0
            elif name in {"b3_shuffled", "b1_real_b2_real_b3_shuffled"}:
            ...
            if name in {"b2_off_b3_real", "b1_real_b2_off_b3_real", "b1_real_b2_off_b3_off"}:
                kwargs["b2_gate_override"] = 0.0
            elif name == "b1_off_b2_real_b3_real":
                kwargs["b1_gate_override"] = 0.0
            elif name == "b1_real_b2_shuffled_b3_real":
                kwargs["b2_recurrent_permutation"] = permutation
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
    ...
    if combined_controls:
        result["combined_system"] = {
            ...
            "b1_marginal_gain": finished["b1_off_b2_real_b3_real"]["validation_loss"] - real["validation_loss"],
            "b2_marginal_gain": finished["b1_real_b2_off_b3_real"]["validation_loss"] - real["validation_loss"],
            "b3_marginal_gain": off["validation_loss"] - real["validation_loss"],
            "b3_gain_with_b2_off": finished["b1_real_b2_off_b3_off"]["validation_loss"] - finished["b1_real_b2_off_b3_real"]["validation_loss"] if "b1_real_b2_off_b3_off" in finished else None,
            "b2_shuffled_gap": finished["b1_real_b2_shuffled_b3_real"]["validation_loss"] - real["validation_loss"],
            "b3_shuffled_gap": finished["b1_real_b2_real_b3_shuffled"]["validation_loss"] - real["validation_loss"],
        }
    return result
...
def attention_diagnostics(model, val_path, link, batch_size=2) -> dict:
    ...
    source = torch.arange(T, device=device).view(1, T)
    lag = query - source
    recurrent_bins = {"b1": B1_LAG_BINS, "b2": B2_RECURRENT_LAG_BINS, "b3": B3_RECURRENT_LAG_BINS}[link]
    local_bins_spec = {"b1": (("0-1", 0, 1),), "b2": B2_LOCAL_LAG_BINS, "b3": B3_LOCAL_LAG_BINS}[link]
    ...
    report = {"link": {"b1": "B12->B1", "b2": "B11->B2", "b3": "B10->B3"}[link],
...
def temporal_gradient_by_lag(model, val_path, link, precision="bf16", gate_override=None, **legacy_kwargs) -> dict:
    ...
    minimum = {"b1": 2, "b2": 32, "b3": 64}[link]
    lag_bins = {"b1": B1_LAG_BINS, "b2": B2_RECURRENT_LAG_BINS, "b3": B3_RECURRENT_LAG_BINS}[link]
    lags = T - 1 - torch.arange(T, device=device)
    bins = {}
    for name, first_lag, last_lag in lag_bins:
        selected = (lags >= first_lag) & (lags <= last_lag)
    ...
    latest = T - 1 - minimum
    gate = {"b1": model.g_rec, "b2": model.g_rec_b2, "b3": model.g_rec_b3}[link]
    effective = {"b1": model.recurrent_scale_b1, "b2": model.recurrent_scale_b2, "b3": model.recurrent_scale_b3}[link]
    report = {"link": {"b1": "B12->B1", "b2": "B11->B2", "b3": "B10->B3"}[link],
...
def attached_writer_gradient_check(model, val_path) -> dict:
    report = {}
    for link, source, latest in (("b1", "h12", 1021), ("b2", "h11", 991), ("b3", "h10", 959)):
        row = temporal_gradient_by_lag(model, val_path, link, gate_override=(0.05 if link == "b3" else None))
        report[{"b1": "b12_to_b1", "b2": "b11_to_b2", "b3": "b10_to_b3"}[link]] = {
...
def kernel_preflight(model, short_tokens, short_targets) -> dict:
    ...
    banks = (model.build_recurrent_bank(values), model.build_recurrent_bank_b2(values), model.build_recurrent_bank_b3(values))
    ...
    try:
        with torch.no_grad():
            active = model.forward_pass(short_tokens, targets=short_targets,
                b1_recurrent_source=values, b2_recurrent_source=values, b3_recurrent_source=values,
    ...
    with torch.no_grad():
        ...
        absent = model.forward_pass(short_tokens, b1_recurrent_source=first["h12"],
                                    b2_recurrent_source=first["h11"], b3_gate_override=0.0)["logits"]
...
def frozen_2d2d_regression(model, source_payload, val_path) -> dict:
    ...
    old.g_rec = model.g_rec; old.g_rec_b2 = model.g_rec_b2
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                for _ in range(3):
                    current = model.forward_pass(x, targets=y, b1_recurrent_source=h12,
                                                 ...
                                                 activation_checkpointing=True)
                    rows.append(current); h10, h11, h12 = current["h10"], current["h11"], current["h12"]
...
def run_preflight(args):
    ...
    attention = {link: attention_diagnostics(model, val_path, link) for link in ("b1", "b2", "b3")}
    ...
    durable_json(output / "gate_diagnostics.json", {"0": {key: zero[key] for key in ("g_rec_b1_raw", "tanh_g_rec_b1", "g_rec_b2_raw", "tanh_g_rec_b2", "g_rec_b3_raw", "tanh_g_rec_b3")}})
    ...
    durable_json(output / "storage_cleanup_manifest.json", {"schema": "exp2d2e_storage_cleanup_manifest_v1", "scientific_or_source_checkpoint_removed": False, "historical_artifacts_removed": False, "workspace_used_bytes_after_cleanup": mount["measured_used_bytes"], "workspace_free_bytes_after_cleanup": mount["measured_free_bytes"], "cleanup_actions": []})
    durable_json(output / "commands_and_runtime.json", {"preflight_command": " ".join(sys.argv), "environment": environment_payload()})
    durable_json(output / "persistent_workspace_audit.json", mount)
    durable_json(output / "runpod_stop_capability.json", stop)
    if not preflight["science_passed"]: raise SystemExit(f"2D2E preflight failed: {checks}")
    print("EXPERIMENT_2D2E_PREFLIGHT_PASS", flush=True)
    return preflight
...
def run_smoke(args):
    ...
    for update in range(1, 4):
        ...
        rows.append({"update": update, "loss": loss.detach().float().item(),
                     ...
                     "recurrent_states_finite": all(bool(torch.isfinite(tensor).all()) for tensor in
                         (first["h10"], first["h11"], first["h12"], second["h10"], second["h11"], second["h12"]))})
        del x, y, cpu_x, cpu_y, first, second, loss
        torch.cuda.empty_cache()
    cache_x, _ = loader.clone().next_batch()
    with torch.no_grad():
    ...
    checks = {"exactly_three_updates": len(rows) == 3,
              ...
              "all_gradient_groups_nonzero": all(all(row["gradient_groups"][name]["finite"] and row["gradient_groups"][name]["nonzero"] for name in ("base", "gate", "b2_gate", "b3_gate")) for row in rows),
    ...
    durable_json(output / "checkpoint_manifest.json", manifest)
    if not audit["passed"]: raise SystemExit(f"2D2E smoke failed: {checks}")
    print("EXPERIMENT_2D2E_SMOKE_PASS", flush=True)
    return audit
...
def milestone_diagnostics(runtime, update, val_path):
    ...
    validation.update(local_update=update, additional_targets=update * GLOBAL_TARGETS,
                      cumulative_2d2_targets=SOURCE_TARGETS + update * GLOBAL_TARGETS)
    attention = {link: attention_diagnostics(runtime.model, val_path, link) for link in ("b1", "b2", "b3")}
    temporal = {link: temporal_gradient_by_lag(runtime.model, val_path, link) for link in ("b1", "b2", "b3")}
    ...
    merge_keyed_json(runtime.output / "gate_diagnostics.json", update,
                     {key: validation[key] for key in ("g_rec_b1_raw", "tanh_g_rec_b1", "g_rec_b2_raw", "tanh_g_rec_b2", "g_rec_b3_raw", "tanh_g_rec_b3")})
...
def reconcile_uncheckpointed_artifacts(output, completed):
    ...
    keyed = ("milestone_validation.json", "paired_controls.json", "gate_diagnostics.json",
             ...
             "b11_to_b2_temporal_gradient.json", "b10_to_b3_temporal_gradient.json")
    for name in keyed:
        path = output / name
...
def _incremental_control(model, x, y, name, derangement=None):
    ...
    ring_max = {"h10": 0, "h11": 0, "h12": 0}; rms = {"b1": [], "b2": [], "b3": []}
    for position in range(length):
...
@torch.no_grad()
def evaluate_incremental(model, val_path, batches=INCREMENTAL_BATCHES) -> dict:
    if int(batches) != 4: raise ValueError("2D2E primary incremental evaluation requires exactly four B64 batches")
    model.eval(); device = next(model.parameters()).device
    loader = legacy.d1.ExplicitShardLoader([val_path], VALIDATION_B, T)
    names = ("all_real", "b3_off", "b3_shuffled", "b3_full_counterfactual", "all_shuffled",
             "b2_off_b3_real", "b1_real_b2_off_b3_off")
    ...
    result = {"controls": controls,
              ...
              "b3_gain_with_b2_off": controls["b1_real_b2_off_b3_off"]["validation_loss"] - controls["b2_off_b3_real"]["validation_loss"],
...
def memory_accounting(incremental=None) -> dict:
    ...
    def one(batch):
        ...
        rings = batch * 3 * 1023 * N_EMBD * element_bytes
        ...
        return {"batch_size": batch, "b1_local_kv_bytes": b1, "b2_local_kv_bytes": b2,
                "b3_local_kv_bytes": b3, "three_recurrent_raw_state_rings_bytes": rings,
    ...
    if incremental is not None:
        ...
        rings = sum(cache[f"{name}_ring_physical_storage"]["actual_bytes"] for name in ("h10", "h11", "h12"))
    ...
    return {"deployment_accounting_dtype": "BF16", "bytes_per_element": element_bytes,
            ...
            "state_limits": {"B1 ordinary historical KV": 1, "B2 ordinary historical KV": 31,
                             "B3 ordinary historical KV": 63, "B10/B11/B12 raw recurrent states each": 1023,
                             "B4-B12 ordinary historical KV each": 1023}}


def classify_result(incremental, stable=True, integrity=True) -> str:
    if not integrity: return "EXPERIMENT 2D2E INVALID"
...
def choose_recommendation(classification, initial_damage):
    if classification == "EXPERIMENT 2D2E INVALID": return "FIX 2D2E INTEGRITY"
...
def self_composition_diagnostic(model, val_path, passes=8, batch_size=2):
    ...
    x, y = cpu_x.to(device), cpu_y.to(device); h10 = h11 = h12 = None; rows = []
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        for index in range(int(passes)):
            ...
            h10, h11, h12 = current["h10"], current["h11"], current["h12"]
...
def _plot_final(output, milestones, training, incremental, attention, temporal, memory, performance, parallel):
    ...
    save(12, lambda ax: (ax.bar(["B1", "B2", "B3"], [marginals["b1_marginal_gain"], marginals["b2_marginal_gain"], marginals["b3_marginal_gain"]]), ax.axhline(0, color="black", linewidth=.8), ax.set(ylabel="Marginal recurrent gain")))
    unit = N_EMBD * 2
    ...
    comparisons = {
        ...
        "2D2E": memory["B1"]["total_experimental_inference_state_bytes"],
    }
...
def _scientific_questions(initial, training, milestones, attention, temporal, parallel, incremental, memory, classification, recommendation):
    ...
    q = {
        ...
        "Q9": training[-1]["tanh_g_rec_b2"], "Q10": training[-1]["tanh_g_rec_b1"],
        ...
        "Q29": parallel["combined_system"]["b2_marginal_gain"],
        "Q30": {"source": SOURCE_B2_GATE_EFFECTIVE, "final": training[-1]["tanh_g_rec_b2"], "change": training[-1]["tanh_g_rec_b2"] - SOURCE_B2_GATE_EFFECTIVE},
        ...
        "Q36": {"gain_with_b2_on": incremental["true_b3_recurrent_gain"], "gain_with_b2_off": incremental["b3_gain_with_b2_off"]},
...
def render_report(summary, audit, questions):
    ...
    lines = [
        "EXPERIMENT 2D2E PRIMARY CLASSIFICATION:", summary["primary_classification"], "",
        ...
        "- Geometry: B1 W2 + B12 lags 2…1023; B2 W32 + B11 lags 32…1023; B3 W64 + B10 lags 64…1023; B4–B12 W1024",
        ...
        "| Update | Targets | Gain | Sequence gap | B1 gate | B2 gate | B3 gate |", "|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for key in sorted(summary["validation_trajectory"], key=int):
        row = summary["validation_trajectory"][key]
        lines.append(f"| {key} | {row['additional_targets']} | {row['b3_recurrent_gain']:.12g} | {row['b3_sequence_gap']:.12g} | {row['tanh_g_rec_b1']:.12g} | {row['tanh_g_rec_b2']:.12g} | {row['tanh_g_rec_b3']:.12g} |")
    ...
    lines += ["# EXPERIMENT 2D2E COMPLETE", ""]
    return "\n".join(lines)
...
def run_finalize(args):
    ...
    attention = {link: attention_diagnostics(model, validation_path(args.data_root), link) for link in ("b1", "b2", "b3")}
    temporal = {link: temporal_gradient_by_lag(model, validation_path(args.data_root), link) for link in ("b1", "b2", "b3")}
    memory = memory_accounting(incremental)
    walls = [row["wall_seconds"] for row in training]
    ...
    for filename, link in (("b12_to_b1_temporal_gradient.json", "b1"), ("b11_to_b2_temporal_gradient.json", "b2"), ("b10_to_b3_temporal_gradient.json", "b3")):
    ...
    checks = {
        ...
        "parameter count 124,475,907": preflight["parameters"]["2d2e_total_parameters"] == TOTAL_PARAMETERS,
        ...
        "B1 and B2 gates resumed; B3 zero initialized": preflight["semantic_diff"]["b1_gate_resumed"] and preflight["semantic_diff"]["b2_gate_resumed"] and preflight["semantic_diff"]["b3_gate_zero"],
        "all source optimizer state resumed; fresh only B3": preflight["source"]["checks"]["source_optimizer_state_preserved"] and preflight["source"]["checks"]["new_b3_optimizer_state_absent"],
    ...
    summary = {"experiment": EXPERIMENT, "primary_classification": classification,
               ...
               "final_gates": {"b1": final_model["tanh_g_rec_b1"], "b2": final_model["tanh_g_rec_b2"], "b3": final_model["tanh_g_rec_b3"]},
    ...
    durable_text(output / "EXPERIMENT_2D2E_FINAL_REPORT.md", render_report(summary, audit, questions))
