"""Architecture kernel for Experiment 2D2E.

2D2E preserves the finalized B12->B1 and B11->B2 recurrent paths from 2D2D
and adds exactly one parameter: the scalar gate for B10->B3 recurrent K/V.
The ordinary windows are B1 W2, B2 W32, B3 W64, and B4-B12 W1024.  Their
complementary recurrent banks cover lags 2..1023, 32..1023, and 64..1023.
...
B10/B11/B12 rings plus at most 1, 31, and 63 historical K/V entries for
B1/B2/B3 respectively; B4-B12 retain ordinary full-context caches.
"""
...
INCREMENTAL_CONTROLS = (
    ...
    "all_shuffled",
    "b2_off_b3_real",
    "b1_off_b2_real_b3_real",
    "b1_real_b2_off_b3_real",
    "b1_real_b2_real_b3_off",
    "b1_real_b2_off_b3_off",
    "b1_real_b2_shuffled_b3_real",
    "b1_real_b2_real_b3_shuffled",
)


@dataclass(frozen=True)
class MirroredIncrementalState:
    """Deployment state for the three graded mirrored recurrent links."""
...
class MirroredFullRecurrentKVGPT(FullB12ToB1RecurrentKVGPT):
    ...
    def forward_pass(
        ...
        """Run one parallel pass and expose exact post-MLP B10/B11/B12 streams."""
        ...
        try:
            ...
            if h10 is None or h11 is None:
                raise RuntimeError("B10/B11 source capture was not reached")
            h12 = residual
    ...
    def forward_multi_pass(
        ...
        if int(num_passes) not in (2, 3):
            raise ValueError("2D2E multi-pass execution permits exactly two or three passes")
        results = [
            self.forward_pass(
                tokens,
                targets=targets,
        ...
        if recurrent_bank is None:
            ...
            coefficient = (
                ...
                if block_index == B1_INDEX
                    ...
                    self._gate_coefficient_b2(local_pre, gate_override)
                    if block_index == B2_INDEX
            ...
            if block_index == B1_INDEX:
                ...
                raise ValueError("special recurrent block must be B1, B2, or B3")
            recurrent_pre, recurrent_weights = self._masked_recurrent_attention(
                query,
                recurrent_key,
                recurrent_value,
    ...
    def incremental_step(
        ...
        shuffled_b2 = control in {
            "all_shuffled",
            "b1_real_b2_shuffled_b3_real",
        }
        shuffled_b3 = control in {
            "all_shuffled",
            "b3_shuffled",
            "b1_real_b2_real_b3_shuffled",
        }
        if shuffled_b1 or shuffled_b2 or shuffled_b3:
            permutation = self._validate_permutation(
        ...
        b1_off = control == "b1_off_b2_real_b3_real"
        ...
        for block_index in range(3, TOTAL_LAYERS):
            ...
            if block_index == B11_INDEX:
                h11 = residual
        if h10 is None or h11 is None:
            raise RuntimeError("incremental B10/B11 capture was not reached")
        h12 = residual
        top = self.base.transformer.ln_f(h12)
        logits = self.base.lm_head(top)
...
Experiment2D2EModel = MirroredFullRecurrentKVGPT
