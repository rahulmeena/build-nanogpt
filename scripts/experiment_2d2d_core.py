"""Architecture kernel for Experiment 2D2D.

2D2D preserves the finalized full-bank B12->B1 path from 2D2B and adds
exactly one mirrored path: older-bank B11->B2 recurrent K/V.  Block 1 keeps
W2 ordinary same-layer context, Block 2 uses W32, and Blocks 3-12 remain
W1024.  B2's local lags 0...31 and recurrent lags 32...1023 form an exact,
non-overlapping temporal partition.
...
31 historical B2 K/V entries, and ordinary upper-layer caches.
"""
...
class MirroredFullRecurrentKVGPT(FullB12ToB1RecurrentKVGPT):
    ...
    def b2_recurrent_mask(
        ...
        # Those lags belong exclusively to B2 local attention in 2D2D.
    ...
    def forward_multi_pass(
        ...
        if int(num_passes) not in (2, 3):
            raise ValueError("2D2D multi-pass execution permits exactly two or three passes")
        results = [
            self.forward_pass(
                tokens,
                targets=targets,
...
Experiment2D2DModel = MirroredFullRecurrentKVGPT
