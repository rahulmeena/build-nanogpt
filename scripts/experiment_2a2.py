#!/usr/bin/env python3
"""Exact single-A100 continuation of Experiment 2A1 from 25M to 100M tokens."""
...
CONFIG_PATH = REPO_ROOT / "configs" / "exp2a2_100m.json"
BRANCH = "codex/experiment-2a2-100m"
START_UPDATE = 48
TARGET_UPDATE = 191
MILESTONES = (96, 191)
...
EXPECTED_MILESTONE_NEXT_SHA256 = {
    96: "8b5f60d4333ba75235e63cb6397dedc31b8105f629e044ae87ca096bf6a57864",
    191: "9f39510b105f068966ef6c052edc015d695827c422da37495fa7c244b965af0b",
}
...
PINNED_UPDATE48_REAL = 5.835339140892029
PINNED_UPDATE48_SHUFFLED = 5.8765912532806395
EXPECTED_HELLASWAG_VAL_SHA256 = (
    "0aa3b88843990f3f10a97b9575c94d7b71fb2205240ba04ae4884d9e9c992588"
)
T19_975 = 2.093024054408263
CLASSIFICATION_RULE = (
    "Let q25, q50, q100 be shuffled-minus-real recovery at 25M, 50M, 100M. "
    "DISAPPEARING if q100<=0 or q100<0.5*q25; SATURATING if q100>0 and "
    "the 50M-to-100M gain is <=25% of max(q50-q25,1e-12); ACCELERATING "
    "if q100>q50>q25 and (q100-q50)>(q50-q25); otherwise STRENGTHENING."
)
CONTINUE_TO_250M_RULE = (
    "YES only if q100>0, real beats shuffled on at least 15 of 20 batches, "
    "q100>=q50, total recovery remains positive, all controls and invariants pass, "
...
def validate_config(config):
    expected = {
        "protocol": "exp2a2_teacher_topdown_100m_v1",
        "seed": a0.SEED,
        ...
        "lr_schedule": "continue original 10B schedule; next global schedule step is 1002",
        "data_start": (
            "restore exact four-rank replay states from verified Experiment 2A1 "
            "update-48 checkpoint"
        ),
    ...
    if mismatches:
        raise SystemExit(f"Experiment 2A2 config mismatch: {mismatches}")
    return config
...
def reconcile_metrics(path, completed_updates, start_update=START_UPDATE):
    ...
    if updates != expected:
        raise SystemExit(f"2A2 metrics history mismatch: {updates} != {expected}")
    trailing = rows[len(prefix):]
    if any(row.get("update", -1) < completed_updates for row in trailing):
        raise SystemExit("2A2 metrics contain duplicate/out-of-order committed rows")
...
def source_metadata_path():
    return REPO_ROOT / "results" / "experiment_2a1_25m" / "metadata.json"


def source_summary_path():
    return REPO_ROOT / "results" / "experiment_2a1_25m" / "result_summary.json"
...
def load_source_metadata():
    ...
    if not metadata_path.is_file() or not summary_path.is_file():
        raise SystemExit("committed Experiment 2A1 25M lineage artifacts are missing")
    metadata = json.loads(metadata_path.read_text())
    summary = json.loads(summary_path.read_text())
    if metadata.get("git_commit") != "529ca93b0a77d91e6aba76f9863f6d7108c6605f":
        raise SystemExit("source 25M metadata commit mismatch")
    if summary.get("completed_updates") != START_UPDATE:
        raise SystemExit("source 25M summary mismatch")
    ...
    if mismatches:
        raise SystemExit(f"live Experiment 2A1 source lineage mismatch: {mismatches}")
    return metadata, summary


def require_clean_tree():
    if a0.git_output("status", "--porcelain"):
        raise SystemExit("2A2 optimizer runs require a clean committed worktree")
    if a0.git_output("branch", "--show-current") != BRANCH:
        raise SystemExit(f"2A2 requires branch {BRANCH}")


def require_pinned_runtime():
    actual = Path(sys.executable).resolve()
    if not PINNED_PYTHON.exists():
        raise SystemExit(f"pinned 2A2 Python is missing: {PINNED_PYTHON}")
    if actual != PINNED_PYTHON.resolve():
        raise SystemExit(
            f"2A2 requires pinned Python {PINNED_PYTHON.resolve()}, got {actual}"
        )
    return str(actual)
...
def continuation_metadata(config, parent_aux, continuation_path):
    ...
    return {
        "experiment": "Experiment 2A2",
        "kind": "exact update-48 to update-191 continuation",
        ...
        "lineage_artifact_sha256": {
            "results/experiment_2a1_25m/config.json": a0.file_sha256(
                REPO_ROOT / "results" / "experiment_2a1_25m" / "config.json"
            ),
            "results/experiment_2a1_25m/metadata.json": a0.file_sha256(
                source_metadata_path()
            ),
            "results/experiment_2a1_25m/result_summary.json": a0.file_sha256(
                source_summary_path()
            ),
        },
        "source_depths": list(a0.SOURCE_DEPTHS),
        ...
        "continue_to_250m_rule": CONTINUE_TO_250M_RULE,
        "config": config,
...
def validate_source_checkpoint(path):
    ...
    if a0.file_sha256(path) != EXPECTED_CONTINUATION_SHA256:
        raise SystemExit("Experiment 2A1 update-48 continuation SHA mismatch")
...
def assert_runtime_contract(student, teacher, optimizer, completed_updates):
    ...
    if sum(parameter.numel() for parameter in student.parameters() if parameter.requires_grad) != 1537:
        raise SystemExit("Experiment 2A2 trainable-parameter count changed")
    nonfinite = [
        name
...
def train_one_update(
    ...
    if not START_UPDATE <= update < TARGET_UPDATE:
        raise SystemExit(f"invalid 2A2 update index: {update}")
...
def evaluate_milestone(student, teacher, symbols, device, completed_updates, full=False):
    ...
    if full != (completed_updates == TARGET_UPDATE):
        raise SystemExit("only update 191 may use the full control matrix")
    ...
    if completed_updates == START_UPDATE:
        if losses["real_feedback"] != PINNED_UPDATE48_REAL:
            raise SystemExit(
                f"update-48 real loss mismatch: {losses['real_feedback']} "
                f"!= {PINNED_UPDATE48_REAL}"
            )
        if losses["shuffled_feedback"] != PINNED_UPDATE48_SHUFFLED:
            raise SystemExit(
                f"update-48 shuffled loss mismatch: {losses['shuffled_feedback']} "
                f"!= {PINNED_UPDATE48_SHUFFLED}"
            )
...
def validate_evaluation_artifact(
    ...
    if completed_updates == START_UPDATE:
        source_milestone_valid = (
            real_value == PINNED_UPDATE48_REAL
            and shuffled_value == PINNED_UPDATE48_SHUFFLED
        )
    source_hashes_valid = (
...
def hellaswag_path(run_dir):
    return Path(run_dir) / "evaluations" / "hellaswag_updates_000191.json"
...
def classify_trajectory(rows):
    if len(rows) != 3:
        raise ValueError("trajectory classification requires exactly three milestones")
    completed = [row.get("completed_updates") for row in rows]
    if completed != [START_UPDATE, *MILESTONES]:
        raise ValueError(
            "trajectory milestones must be ordered exactly as updates 48,96,191"
        )
    values = []
    for row in rows:
    ...
    q25, q50, q100 = values
    if q100 <= 0 or (q25 > 0 and q100 < 0.5 * q25):
        label = "MEMORY SIGNAL DISAPPEARING"
    elif q100 > 0 and (q100 - q50) <= 0.25 * max(q50 - q25, 1e-12):
        label = "MEMORY SIGNAL SATURATING"
    elif q100 > q50 > q25 and (q100 - q50) > (q50 - q25):
        label = "MEMORY SIGNAL ACCELERATING"
    else:
...
def validate_training_rows(rows, completed_updates, expected_hashes):
    ...
    if [row.get("update") for row in rows] != expected_updates:
        raise SystemExit("2A2 training rows are missing, duplicated, or out of order")
    for row in rows:
        update = row["update"]
        expected = {
            "completed_updates": update + 1,
...
def load_new_checkpoint(
    ...
    if preview_completed not in MILESTONES:
        raise SystemExit(
            f"2A2 resume checkpoint is not a milestone: {preview_completed}"
        )
    ...
    if completed not in MILESTONES:
        raise SystemExit(f"2A2 resume checkpoint is not a milestone: {completed}")
...
def run_preflight(args, device):
    ...
    report = {
        "kind": "Experiment 2A2 optimizer-free continuation preflight",
        "git_commit": a0.git_output("rev-parse", "HEAD"),
        ...
        "canonical_update48_evaluation": source_evaluation,
        "canonical_update48_evaluation_state_isolation": evaluation_isolation,
...
def validate_prior_chain(run_dir, completed, metadata, symbols, parent_aux):
    ...
    if not source_causality_path.is_file() or not validate_causality_record(
        ...
        raise SystemExit("resume update-48 causality audit is missing or invalid")
    checkpoint_records = {}
...
def final_summary(
    ...
    if [row.get("completed_updates") for row in historical] != [10, 20, 29, 48]:
        raise SystemExit("audited Experiment 2A1 historical trajectory is invalid")
    for source_row, current_row in zip(historical[-1:], trajectory_rows[:1]):
        ...
        if any(source_row.get(key) != current_row.get(key) for key in exact_fields):
            raise SystemExit("update-48 continuity evaluation disagrees with audited history")
    ...
    q25, q50, q100 = classification["values"]
    ...
    interval_training = {}
    for lower, upper in ((START_UPDATE, MILESTONES[0]), (MILESTONES[0], TARGET_UPDATE)):
    ...
    summary = {
        ...
        * a0.GLOBAL_BATCH_TOKENS,
        ...
        "continue_to_250m": {
            "answer": "YES" if continue_to_250m else "NO",
            "rule": CONTINUE_TO_250M_RULE,
            "passed": continue_to_250m,
        },
        ...
        "terminal_line": "EXPERIMENT 2A2 100M COMPLETE",
...
def run_continuation(args, device):
    ...
    if set(restart_records) != {"96"} or not all(
    ...
    if set(causality_records) != {"48", "96", "191"} or not all(
...
def main():
    ...
    continuation = subparsers.add_parser("continue-100m")
