"""Architecture kernel for Experiment 2D2F.

2D2F is the clean sibling of 2D2E.  It starts from final 2D2D, physically
removes the B11->B2 recurrent path and its scalar, keeps B2 as local W32,
and adds exactly one fresh parameter: the B10->B3 recurrent gate.  B1 keeps
the finalized B12->B1 path.  The two recurrent banks therefore cover lags
2..1023 and 64..1023, with no B11 writer ring or B2 recurrent serving path.

The incremental path stores raw B10/B12 rings, at most 1/31/63 historical
K/V entries for B1/B2/B3, and ordinary full-context caches for B4-B12.
"""
...
MAX_RECURRENT_ENTRIES = B3_MAX_RECURRENT_ENTRIES
...
class MirroredIncrementalState:
    """Deployment state for B12->B1 and B10->B3 only."""
...
class MirroredFullRecurrentKVGPT(FullB12ToB1RecurrentKVGPT):
    """Final 2D2D weights with B2 recurrence removed and B10->B3 added."""

    def __init__(self, base: nn.Module):
        super().__init__(base)
        # The sole new tensor; the source B2 gate is intentionally not registered.
    ...
    def _parallel_block2(
        ...
        """Run physically local-only B2 W32 attention."""
        ...
        combined = local_pre.transpose(1, 2).contiguous().view(batch, length, channels)
    ...
    def forward_pass(
        ...
        """Run one parallel pass and expose exact post-MLP B10/B12 streams."""
        ...
        try:
            ...
            if use_checkpoint and not return_diagnostics:
                residual = checkpoint(
                    lambda value: self._parallel_block2(value, False)[0],
                    residual,
                    use_reentrant=False,
                    preserve_rng_state=False,
                )
            else:
                residual, b2_diagnostics = self._parallel_block2(residual, return_diagnostics)
    ...
    def forward_multi_pass(
        ...
        if int(num_passes) not in (2, 3):
            raise ValueError("2D2F multi-pass execution permits exactly two or three passes")
        results = [
            self.forward_pass(
                tokens,
                targets=targets,
    ...
    def _validate_incremental_state(self, state: MirroredIncrementalState) -> None:
        ...
        b3 = (
            upper
            if state.b3_full_cache
            else min(state.position, B3_LOCAL_WINDOW - 1)
        )
        expected = (
    ...
    def _incremental_special_block(
        ...
        if recurrent_bank is None:
            ...
            if block_index == B1_INDEX:
                ...
                raise ValueError("special recurrent block must be B1 or B3")
            recurrent_pre, recurrent_weights = self._masked_recurrent_attention(
                query,
                recurrent_key,
                recurrent_value,
    ...
    def incremental_step(
        ...
        wants_full_b3 = control == "b3_full_counterfactual"
        if state.b3_full_cache != wants_full_b3:
            raise ValueError("incremental state B3 cache geometry does not match control")
        ...
        shuffled_b3 = control in {"all_shuffled", "b3_shuffled"}
        if shuffled_b1 or shuffled_b3:
            permutation = self._validate_permutation(
        ...
        residual, next_b2_cache = self._incremental_ordinary_block(
            residual,
            B2_INDEX,
        ...
        b3_diag = None
        if wants_full_b3:
            residual, next_b3_cache = self._incremental_ordinary_block(
                residual,
                B3_INDEX,
            ...
            b3_off = control in {"b3_off", "b1_real_b3_off"}
            residual, next_b3_cache, b3_diag = self._incremental_special_block(
                residual,
                B3_INDEX,
                state.caches[B3_INDEX],
                None if b3_off else b3_bank,
                permutation if shuffled_b3 else None,
                0.0 if b3_off else b3_gate_override,
                B3_LOCAL_WINDOW - 1,
                return_diagnostics,
                diagnostic_attention_weights,
            )

        updated_caches = [next_b1_cache, next_b2_cache, next_b3_cache]
        ...
        for block_index in range(3, TOTAL_LAYERS):
            ...
            if block_index == B10_INDEX:
                h10 = residual
        if h10 is None:
            raise RuntimeError("incremental B10 capture was not reached")
        h12 = residual
        top = self.base.transformer.ln_f(h12)
        logits = self.base.lm_head(top)
        ...
        return logits, next_state, {
            ...
            "control": control,
            "b1": b1_diag,
            "b2": {"recurrent_path_present": False},
            "b3": b3_diag,
    ...
    def incremental_cache_audit(self, state: MirroredIncrementalState) -> dict:
        ...
        b3_limit = (
            upper_limit if state.b3_full_cache else B3_LOCAL_WINDOW - 1
        )
        passed = (
            lengths[0] <= 1
            and lengths[1] <= B2_LOCAL_WINDOW - 1
            and lengths[2] <= b3_limit
            and all(length <= upper_limit for length in lengths[3:])
        ...
        return {
            ...
            "b4_b12_historical_kv_limit": upper_limit,
            ...
            "b3_full_counterfactual": state.b3_full_cache,
...
Experiment2D2FModel = MirroredFullRecurrentKVGPT
