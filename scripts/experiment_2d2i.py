#!/usr/bin/env python3
"""Experiment 2D2I: B4 W128 plus the older B9-to-B4 recurrent K/V bank.

The finalized 2D2E checkpoint is the immutable source. This driver restores
its complete model, optimizer, loader, RNG, and cursor state; preserves the
trained B12-to-B1, B11-to-B2, and B10-to-B3 paths; changes only B4 from W1024
to W128; and adds exactly one scalar gate for B9-to-B4 recurrence at lags
128..1023.
"""
...
import experiment_2d2e as source_driver  # noqa: E402
import experiment_2d2e_core as source_core  # noqa: E402
from experiment_2d2i_core import (  # noqa: E402
    BANK_MODES,
    B2_LOCAL_WINDOW,
...
EXPERIMENT = "2D2I"
PROTOCOL = "exp2d2i_b4_w128_b9_recurrent_896_v1"
BRANCH = "experiment-2d2i-b4-w128-b9-recurrent"
FROZEN_TAG = "experiment-2d2e-b3-w64-b10-recurrent-960-final"
FROZEN_COMMIT = "406bef0dc0f375d6783cfc364a935e62bb54d982"
CONFIG_PATH = REPO_ROOT / "configs" / "exp2d2i_b4_w128_b9_recurrent_896.json"
OUTPUT_NAME = "experiment_2d2i_b4_w128_b9_recurrent"
CHECKPOINT_SCHEMA = "exp2d2i_b4_w128_b9_recurrent_896_checkpoint_v1"
SOURCE_SCHEMA = "exp2d2e_b3_w64_b10_recurrent_960_checkpoint_v1"
SOURCE_SHA256 = "dea5e76b55d1ad7281fe3cf3893713392343b875182ba186a0049904e61de790"
SOURCE_BYTES = 1_493_942_757
SOURCE_UPDATES = 191
SOURCE_TARGETS = 350_748_672
SOURCE_GATE_RAW = 0.17640022933483124
SOURCE_GATE_EFFECTIVE = 0.17459304630756378
SOURCE_B2_GATE_RAW = 0.007704777177423239
SOURCE_B2_GATE_EFFECTIVE = 0.007704624906182289
SOURCE_B3_GATE_RAW = 0.017463035881519318
SOURCE_B3_GATE_EFFECTIVE = 0.0174612607806921
SOURCE_NEXT_BATCH_SHA256 = "91fa2cae4e6e52cfddd2b470175ec704f0548b447f02861917ec548736fe18e7"
SOURCE_NEXT_STREAM_SHA256 = "4da6fed71755e523030a2d8e9e7cc96d19691a8c9b3ac8c490426bafe3d44e82"
PERSISTENT_VOLUME_IDENTITY = "yhzyb27fb5"
VOLUME_CAPACITY_DECIMAL_BYTES = 100_000_000_000
PERSISTENT_SAFETY_MARGIN_BYTES = 8_000_000_000
MASTER_STORAGE_PREFLIGHT = Path(
    "/workspace/parallel_2d2_master/storage_preflight.json"
)
PERSISTENT_FINAL_CHECKPOINT = Path(
...
TOTAL_PARAMETERS = 124_475_908
SOURCE_PARAMETERS = 124_475_907
BASE_LR = 3e-5
GATE_LR = 3e-4
GRAD_CLIP = 1.0
...
REQUIRED_ARTIFACTS = (
    "EXPERIMENT_2D2I_FINAL_REPORT.md",
    "FINAL_REPORT.md",
    "FINAL_AUDIT.json",
    "result_summary.json",
...
REQUIRED_PLOTS = tuple(f"P{index}_{name}.png" for index, name in enumerate((
    "all_real_b4_off_validation_ce",
    "b4_recurrent_gain",
    "b4_sequence_gap",
    "b1_b2_b3_b4_gate_trajectories",
    "b4_local_attention_by_lag",
    "b4_recurrent_attention_by_lag",
    "b4_normalized_recurrent_attention_density",
    "b4_per_head_mean_recurrent_lag",
    "b9_to_b4_writer_gradient",
    "b1_b2_b3_b4_temporal_distance_comparison",
    "parallel_vs_true_incremental_b4_gain",
    "b1_b2_b3_b4_marginal_recurrent_gains",
...
def gradient_group_report(model):
    """Report base and all four scalar-gate gradients independently."""

    base = [
        parameter.grad
        for name, parameter in model.named_parameters()
        if name not in {"g_rec", "g_rec_b2", "g_rec_b3", "g_rec_b4"}
        and parameter.grad is not None
    ]
    b1_gate = [] if model.g_rec.grad is None else [model.g_rec.grad]
    b2_gate = [] if model.g_rec_b2.grad is None else [model.g_rec_b2.grad]
    b3_gate = [] if model.g_rec_b3.grad is None else [model.g_rec_b3.grad]
    b4_gate = [] if model.g_rec_b4.grad is None else [model.g_rec_b4.grad]
    report = {}
    for name, values in (
        ("base", base),
        ("gate", b1_gate),
...
def require_config() -> dict:
    ...
    checks = {
        ...
        "frozen_tag": config.get("frozen_2d2e_tag") == FROZEN_TAG,
        "frozen_commit": config.get("frozen_2d2e_commit") == FROZEN_COMMIT,
        "source_sha": config.get("source_checkpoint_sha256") == SOURCE_SHA256,
        "source_bytes": config.get("source_checkpoint_bytes") == SOURCE_BYTES,
        "source_state": config["source_2d2e"]["completed_updates"] == SOURCE_UPDATES
        and config["source_2d2e"]["completed_targets"] == SOURCE_TARGETS
        and config["source_2d2e"]["raw_g_rec_b1"] == SOURCE_GATE_RAW
        and config["source_2d2e"]["raw_g_rec_b2"] == SOURCE_B2_GATE_RAW
        and config["source_2d2e"]["raw_g_rec_b3"] == SOURCE_B3_GATE_RAW
        and config["source_2d2e"]["next_global_batch_sha256"]
        ...
        and config["architecture"]["b2"]["recurrent_min_lag"]
        == B2_RECURRENT_MIN_LAG
        and config["architecture"]["b2"]["recurrent_max_lag"]
        == RECURRENT_MAX_LAG
        and config["architecture"]["b2"]["maximum_recurrent_entries"]
        == B2_MAX_RECURRENT_ENTRIES
        ...
        == B3_MAX_RECURRENT_ENTRIES
        and config["architecture"]["b4"]["local_window"] == B4_WINDOW
        and config["architecture"]["b4"]["recurrent_min_lag"]
        == B4_RECURRENT_MIN_LAG
        and config["architecture"]["b4"]["recurrent_max_lag"]
        == RECURRENT_MAX_LAG
        and config["architecture"]["b4"]["maximum_recurrent_entries"]
        == B4_MAX_RECURRENT_ENTRIES,
        "one_parameter": config["architecture"]["new_parameter_count_vs_2d2e"] == 1
        and config["architecture"]["new_parameters_vs_2d2e"] == ["g_rec_b4"]
        ...
        "new_b4_gate_optimizer": config["training"]["optimizer"][
            "new_b4_gate_state_fresh"
        ] is True
        and config["training"]["optimizer"]["new_b4_gate_lr"] == GATE_LR
        ...
        "hardware": config["hardware"]["pod_id"] == "7i2zyd53ytspwz"
        and config["hardware"]["pod_name"] == "empirical_tan_panda"
        and config["hardware"]["gpu_count"] == 4
        and config["hardware"]["assigned_cuda_visible_device"] == 3,
    }
    if not all(checks.values()):
        raise SystemExit(f"2D2I preregistration mismatch: {checks}")
    return config
...
def workspace_mount_audit(output_dir, run_root, supplied_identity) -> dict:
    ...
    if output != expected_output:
        raise SystemExit(f"2D2I result directory must be exactly {expected_output}")
    if not str(output).startswith("/workspace/"):
        raise SystemExit("2D2I result artifacts must live under /workspace")
    row = subprocess.check_output(
        ["findmnt", "-T", "/workspace", "-n", "-o", "TARGET,SOURCE,FSTYPE"],
        text=True,
    ).strip().split(maxsplit=2)
    target, source, filesystem = row
    quota_bytes = VOLUME_CAPACITY_DECIMAL_BYTES
    safety_margin_bytes = PERSISTENT_SAFETY_MARGIN_BYTES
    # The master preflight performs the sole authorized full-volume logical-size
    # scan.  Per-lane commands consume that immutable reservation instead of
    # interpreting the backend-aggregate FUSE df value as this volume's quota or
    # launching four duplicate recursive scans.
    master_storage = MASTER_STORAGE_PREFLIGHT
    if not master_storage.is_file():
        raise SystemExit("shared master storage preflight is missing")
    storage_payload = read_json(master_storage)
    free_bytes = int(storage_payload.get("logical_quota_free_decimal_bytes", -1))
    logical_workspace = storage_payload.get("logical_workspace", {})
    used_bytes = int(logical_workspace.get("logical_file_bytes", -1))
    projected = storage_payload.get("projected", {})
    master_checks = storage_payload.get("checks", {})
    run_row = subprocess.check_output(
        ["findmnt", "-T", str(run), "-n", "-o", "TARGET,SOURCE,FSTYPE"], text=True
    ).strip().split(maxsplit=2)
    run_target, run_source, run_filesystem = run_row
    required_final_bytes = SOURCE_BYTES
    required_free_bytes = required_final_bytes + safety_margin_bytes
    checks = {
        ...
        "run_root_is_node_local": not str(run).startswith("/workspace/")
        and run_filesystem != "fuse",
        "master_storage_preflight_passed": storage_payload.get("passed") is True,
        "master_quota_exact": storage_payload.get(
            "network_volume_capacity_decimal_bytes"
        ) == quota_bytes,
        "master_projection_reserved_all_lane_finals": master_checks.get(
            "persistent_projection_with_margin_fits"
        ) is True,
        "preregistered_safety_margin_exact": projected.get(
            "persistent_safety_margin_bytes"
        ) == safety_margin_bytes,
        "logical_quota_snapshot_consistent": used_bytes >= 0
        and free_bytes == quota_bytes - used_bytes,
        "lane_final_plus_reserved_margin_fits": free_bytes >= required_free_bytes,
    }
    if not all(checks.values()):
    ...
    return {
        ...
        "run_root": str(run),
        "run_root_mount": {"target": run_target, "source": run_source, "filesystem": run_filesystem},
        "shared_master_storage_manifest": str(master_storage),
        "configured_quota_bytes": quota_bytes,
        "quota_source": "master logical-volume scan; FUSE df is non-authoritative",
        "master_logical_used_bytes_at_preflight": used_bytes,
        "master_logical_free_bytes_at_preflight": free_bytes,
        # Backward-compatible artifact keys. These are explicitly the master's
        # logical preflight snapshot, not fresh per-command measurements.
        "measured_used_bytes": used_bytes,
        "measured_free_bytes": free_bytes,
        "required_final_checkpoint_bytes": required_final_bytes,
        "preregistered_safety_margin_bytes": safety_margin_bytes,
...
def authenticated_stop_audit(args) -> dict:
    ...
    checks = {
        "authenticated_probe": payload.get("authenticated") is True,
        "pod_id": payload.get("pod_id") == args.pod_id == "7i2zyd53ytspwz",
        "pod_name": payload.get("pod_name") == args.pod_name == "empirical_tan_panda",
        "gpu_count": payload.get("gpu_count") == 4,
        "runtime_running": payload.get("runtime_status") == "running",
        "exact_stop_target": payload.get("pod_id") == "7i2zyd53ytspwz",
        "volume_preserved": payload.get("volume_id") == PERSISTENT_VOLUME_IDENTITY
        and payload.get("network_volume_preservation_required") is True,
        "delete_forbidden": payload.get("pod_delete_forbidden") is True,
...
def configure_optimizer(model, device_type="cuda", include_new_gate=True):
    """Recreate 2D2E groups, then optionally add only the fresh B4 gate."""

    base_decay, base_nodecay = [], []
    for name, parameter in model.named_parameters():
        if not parameter.requires_grad or name in {
            "g_rec", "g_rec_b2", "g_rec_b3", "g_rec_b4"
        }:
            continue
    ...
    report = {
        ...
        "new_b4_gate_state_fresh": bool(include_new_gate),
...
def load_source_bundle(source_checkpoint, device, restore_rng=False):
    ...
    if observed_sha != SOURCE_SHA256 or path.stat().st_size != SOURCE_BYTES:
        raise SystemExit("2D2E scientific source checkpoint identity mismatch")
    sha_sidecar = path.with_suffix(path.suffix + ".sha256")
    verification_sidecar = path.with_suffix(path.suffix + ".verification.json")
    if not sha_sidecar.is_file() or not verification_sidecar.is_file():
        raise SystemExit("2D2E checkpoint sidecars missing")
    if sha_sidecar.read_text().split()[0] != SOURCE_SHA256:
        raise SystemExit("2D2E checkpoint SHA sidecar mismatch")
    ...
    required = {
        ...
        "g_rec_b3",
        "optimizer",
        "completed_2d2e_updates",
        "processed_2d2e_targets",
    ...
    checks = {
        ...
        "updates": payload.get("completed_2d2e_updates") == SOURCE_UPDATES,
    ...
    if not all(checks.values()):
        raise SystemExit(f"2D2E source schema mismatch: {checks}")
    ...
    model.g_rec_b2 = source_model.g_rec_b2
    model.g_rec_b3 = source_model.g_rec_b3
    ...
    optimizer_report.update(
        {
            ...
            "new_b4_gate_state_fresh": model.g_rec_b4 not in optimizer.state,
        }
    )
    ...
    checks.update(
        {
            ...
            "b2_gate_effective": model.recurrent_scale_b2.detach().float().item()
            == SOURCE_B2_GATE_EFFECTIVE,
            "b3_gate_effective": model.recurrent_scale_b3.detach().float().item()
            == SOURCE_B3_GATE_EFFECTIVE,
            "b4_gate_zero": model.g_rec_b4.detach().float().item() == 0.0,
            ...
            "new_b4_optimizer_state_absent": model.g_rec_b4 not in optimizer.state,
    ...
    if not checks["passed"]:
        raise SystemExit(f"2D2E source strict reopen failed: {checks}")
    rng_fingerprints = {
...
def parameter_manifest(model, source_payload) -> dict:
    ...
    comparable_without_new = [
        ...
        if row["name"] != "g_rec_b4"
    ]
    source_keys = list(source_payload["model"])
    target_keys_without_new = [key for key in model.state_dict() if key != "g_rec_b4"]
    new_rows = [row for row in inventory if row["name"] == "g_rec_b4"]
    report = {
        "2d2e_named_parameter_inventory": source_inventory,
        "2d2i_named_parameter_inventory": inventory,
        ...
        "2d2e_total_parameters": sum(row["numel"] for row in source_inventory),
        "2d2i_total_parameters": sum(row["numel"] for row in inventory),
        "new_parameters_vs_2d2e": new_rows,
        "new_parameter_count_vs_2d2e": len(new_rows),
    ...
    report["checks"] = {
        ...
        "source_total": report["2d2e_total_parameters"] == SOURCE_PARAMETERS,
        "target_total": report["2d2i_total_parameters"] == TOTAL_PARAMETERS,
        "exactly_one_new": report["new_parameter_count_vs_2d2e"] == 1,
        "new_scalar_exact": len(new_rows) == 1
        and new_rows[0]["name"] == "g_rec_b4"
    ...
    return {
        ...
        "links": {
            ...
            "B11_to_B2": {
                ...
                "gate": "resumed tanh(g_rec_b2)",
                "local_window": B2_WINDOW,
                "recurrent_lags": "32...1023",
                "maximum_recurrent_entries": B2_MAX_RECURRENT_ENTRIES,
            },
            "B10_to_B3": {
                "source": "B10 post-MLP residual immediately before B11",
                "destination": "B3 attention",
                "gate": "resumed tanh(g_rec_b3)",
                "local_window": B3_WINDOW,
                "recurrent_lags": "64...1023",
                "maximum_recurrent_entries": B3_MAX_RECURRENT_ENTRIES,
            },
            "B9_to_B4": {
                "source": "B9 post-MLP residual immediately before B10",
                "destination": "B4 attention",
                "gate": "new tanh(g_rec_b4)",
                "local_window": B4_WINDOW,
                "recurrent_lags": "128...1023",
                "maximum_recurrent_entries": B4_MAX_RECURRENT_ENTRIES,
            },
        },
        "b1_local_window": B1_LOCAL_WINDOW,
        "b2_local_window": B2_WINDOW,
        "b3_local_window": B3_WINDOW,
        "b4_local_window": B4_WINDOW,
        "b5_b12_windows": [T] * 8,
        "b1_recurrent_source_set": "max(0,t-1023)...t-2 inclusive",
        "b2_recurrent_source_set": "max(0,t-1023)...t-32 inclusive",
        "b3_recurrent_source_set": "max(0,t-1023)...t-64 inclusive",
        "b4_recurrent_source_set": "max(0,t-1023)...t-128 inclusive",
        "b1_recurrent_min_lag": 2,
        "b2_recurrent_min_lag": B2_RECURRENT_MIN_LAG,
        "b3_recurrent_min_lag": B3_RECURRENT_MIN_LAG,
        "b4_recurrent_min_lag": B4_RECURRENT_MIN_LAG,
        "recurrent_max_lag": RECURRENT_MAX_LAG,
        "b1_maximum_recurrent_entries": 1022,
        "b2_maximum_recurrent_entries": B2_MAX_RECURRENT_ENTRIES,
        "b3_maximum_recurrent_entries": B3_MAX_RECURRENT_ENTRIES,
        "b4_maximum_recurrent_entries": B4_MAX_RECURRENT_ENTRIES,
        "b1_local_positions": ["t-1", "t"],
        "b2_local_positions": ["t-31", "...", "t"],
        "b3_local_positions": ["t-63", "...", "t"],
        "b4_local_positions": ["t-127", "...", "t"],
        ...
        "incremental_b4_historical_kv_capacity": 127,
        "incremental_b5_b12_historical_kv_capacity": 1023,
        "incremental_b9_raw_residual_capacity": RECURRENT_RING_CAPACITY,
        ...
        "parallel_source_tensor_shapes": {"h9": "[B,T,C]", "h10": "[B,T,C]", "h11": "[B,T,C]", "h12": "[B,T,C]"},
        ...
        "forbidden_modules_absent": {
            ...
            "additional_link_beyond_B9_to_B4": True,
...
def matched_2d2c_reference_manifest() -> dict:
    return {
        "schema": "exp2d2i_frozen_2d2c_matched_reference_v1",
        ...
        **copy.deepcopy(FROZEN_2D2C_REFERENCE),
        ...
        "sole_scientific_difference": (
            ...
            "2D2I local W32 plus recurrent lags 32..1023"
        ),
    }
...
def frozen_2d2c_optimizer_audit(current_report) -> dict:
    ...
    return {
        ...
        "current_2d2i_optimizer_report": current_report,
        "checks": checks,
        "passed": all(checks.values()),
    }
...
def initialize_matched_data_replay_audit(source_audit, source_stream, scientific_stream):
    return {
        "schema": "exp2d2i_matched_2d2c_data_replay_audit_v1",
        ...
        "unavailable_comparisons": (
            ...
            "is compared in memory without creating extra 2D2I checkpoints."
        ),
...
def record_matched_replay_cursor(
    ...
    audit["checkpoint_cursor_comparisons"][str(update)] = {
        ...
        "observed_2d2i_next_global_batch_sha256": observed_batch,
        "expected_2d2c_next_global_batch_stream_sha256": reference[
            "next_stream_sha256"
        ],
        "observed_2d2i_next_global_batch_stream_sha256": observed_stream,
...
def evaluate_parallel(
    ...
    for batch_index in range(batches):
        ...
        for name in names:
            ...
            del output, tensor
            torch.cuda.empty_cache()
        print(f"2D2I validation batch={batch_index + 1:02d}/{batches}", flush=True)
        del x, y, cpu_x, cpu_y
        torch.cuda.empty_cache()
...
def probe_microbatch(model, optimizer, shards, device, candidates=(32, 16, 8, 4, 2)):
    ...
    for candidate in candidates:
        ...
        try:
            ...
            gc.collect()
            torch.cuda.empty_cache()
    raise SystemExit(f"no safe 2D2I microbatch: {attempts}")
...
def run_preflight(args):
    ...
    if not optimizer_match["passed"]:
        raise SystemExit(
            f"2D2I optimizer differs from frozen matched 2D2C: {optimizer_match['checks']}"
        )
    ...
    durable_json(
        ...
        {
            "schema": "exp2d2i_storage_cleanup_manifest_v1",
            "preflight_cleanup": {
                "action_provenance": (
    ...
    if (
        ...
        raise SystemExit(
            "2D2I did not preserve frozen one-GPU 2D2C microbatch/reduction geometry"
        )
    ...
    if not preflight["science_passed"]:
        raise SystemExit(f"2D2I preflight failed: {science_checks}")
    print("EXPERIMENT_2D2I_PREFLIGHT_PASS", flush=True)
    return preflight
...
def checkpoint_payload(model, optimizer, loader, training_state, metadata, accumulation):
    return {
        ...
        "g_rec_b4": model.g_rec_b4.detach().cpu().clone(),
        "optimizer": optimizer.state_dict(),
        "completed_2d2i_updates": training_state["completed_2d2i_updates"],
        "processed_2d2i_targets": training_state["processed_2d2i_targets"],
        "cumulative_2d2_targets": training_state["cumulative_2d2_targets"],
        "source_2d2e_updates": SOURCE_UPDATES,
        "source_2d2e_targets": SOURCE_TARGETS,
...
def strict_reopen_checkpoint(
    ...
    required = {
        ...
        "g_rec_b4",
        "optimizer",
        "completed_2d2i_updates",
        "processed_2d2i_targets",
        "cumulative_2d2_targets",
        "source_2d2e_updates",
        "source_2d2e_targets",
        "training_state",
    ...
    checks = {
        ...
        "updates": reopened.get("completed_2d2i_updates")
        == training_state["completed_2d2i_updates"],
        "additional_targets": reopened.get("processed_2d2i_targets")
        == training_state["processed_2d2i_targets"],
        ...
        "b4_gate_duplicate": torch.equal(
            reopened.get("g_rec_b4"), model.g_rec_b4.detach().cpu()
        ),
    }
    if not all(checks.values()):
    ...
    verification = {
        ...
        "bytes": path.stat().st_size,
        "completed_2d2i_updates": reopened["completed_2d2i_updates"],
...
def run_smoke(args):
    ...
    for update in range(1, 4):
        ...
        torch.cuda.empty_cache()
    smoke_path = Path(args.run_root).resolve() / "smoke" / "2d2i_disposable_update_0003.pt"
    smoke_path.parent.mkdir(parents=True, exist_ok=True)
    if smoke_path.exists():
        raise SystemExit(f"refusing to overwrite disposable smoke checkpoint: {smoke_path}")
    probe_loader = loader.clone(); probe_x, probe_y = probe_loader.next_batch()
    expected_next_microbatch = batch_hash(probe_x, probe_y)
    smoke_rng = capture_rng_state()
    smoke_payload = {
        "schema": "exp2d2i_disposable_smoke_checkpoint_v1",
        "model": model.state_dict(), "optimizer": optimizer.state_dict(),
        "loader_state": loader.state_dict(), "rng_state": smoke_rng,
        "completed_smoke_updates": 3,
        "next_microbatch_sha256": expected_next_microbatch,
    }
    torch.save(smoke_payload, smoke_path)
    smoke_sha = file_sha256(smoke_path)
    reopened = legacy.d0.torch_load(smoke_path, mmap=False)
    model.load_state_dict(reopened["model"], strict=True)
    optimizer.load_state_dict(reopened["optimizer"])
    reopened_loader = legacy.d1.ExplicitShardLoader(
        reopened["loader_state"]["shards"], reopened["loader_state"]["batch_size"],
        reopened["loader_state"]["sequence_length"], state=reopened["loader_state"])
    actual_x, actual_y = reopened_loader.next_batch()
    restore_rng_state(reopened["rng_state"])
    smoke_reload = {
        "checkpoint_sha256": smoke_sha,
        "bytes": smoke_path.stat().st_size,
        "schema_exact": reopened.get("schema") == "exp2d2i_disposable_smoke_checkpoint_v1",
        "updates_exact": reopened.get("completed_smoke_updates") == 3,
        "model_finite": model_finite(model), "optimizer_finite": optimizer_finite(optimizer),
        "next_microbatch_exact": batch_hash(actual_x, actual_y) == expected_next_microbatch,
        "rng_exact": {name: state_fingerprint(value) for name, value in capture_rng_state().items()}
        == {name: state_fingerprint(value) for name, value in smoke_rng.items()},
    }
    smoke_reload["passed"] = all(value for key, value in smoke_reload.items()
                                         if key not in {"checkpoint_sha256", "bytes"})
    del smoke_payload, reopened, probe_x, probe_y, actual_x, actual_y
    smoke_bytes = smoke_path.stat().st_size
    smoke_path.unlink()
    ...
    state = {
        "completed_2d2i_updates": 3,
        "processed_2d2i_targets": 3 * smoke_batch * T,
    ...
    if not audit["passed"]:
        raise SystemExit(f"2D2I smoke failed: {checks}")
    ...
    durable_json(output / "smoke_audit.json", audit)
    print("EXPERIMENT_2D2I_SMOKE_PASS", flush=True)
    return audit
...
def training_metadata(args, preflight, micro_batch, accumulation) -> dict:
    return {
        ...
        "implementation_git_commit": git_output("rev-parse", "HEAD"),
        "implementation_fingerprint": preflight["implementation_fingerprint"],
        "frozen_2d2e_commit": FROZEN_COMMIT,
        ...
        "optimizer": {
            ...
            "base_lr": BASE_LR,
            "gate_lr": GATE_LR,
            "existing_b2_gate_lr": GATE_LR,
            "existing_b3_gate_lr": GATE_LR,
            "new_b4_gate_lr": GATE_LR,
...
def initialize_runtime(args):
    ...
    if args.resume:
        ...
        if state["completed_2d2i_updates"] != FORCED_RESTART_UPDATE:
        ...
        restart = {
            ...
            "completed_2d2i_updates": state["completed_2d2i_updates"],
        ...
        state = {
            "completed_2d2i_updates": 0,
            "processed_2d2i_targets": 0,
        ...
        saved_pid = None
    if state["completed_2d2i_updates"] == 0:
        if model.g_rec.detach().float().item() != SOURCE_GATE_RAW:
            raise SystemExit("scientific source gate was not restored exactly")
        if model.g_rec_b2.detach().float().item() != SOURCE_B2_GATE_RAW:
            raise SystemExit("scientific source B2 gate was not restored exactly")
        if model.g_rec_b3.detach().float().item() != SOURCE_B3_GATE_RAW:
            raise SystemExit("scientific source B3 gate was not restored exactly")
        if model.g_rec_b4.detach().float().item() != 0.0:
            raise SystemExit("new B4 gate was not initialized at zero")
...
def train_one_update(runtime, update):
    ...
    for micro_index in range(runtime.accumulation):
        ...
        if micro_index == runtime.accumulation - 1:
            final_h9_rms = results[-1]["h9"].detach().float().square().mean().sqrt().item()
            ...
            final_b4_recurrent_rms = results[-1]["diagnostics"]["b4"][
                "recurrent_output_rms"
            ].detach().float().item()
        del x, y, cpu_x, cpu_y, results, source_h9, source_h10, source_h11, source_h12, weighted, scaled, current
    if not gradients_finite(model):
        raise SystemExit("nonfinite gradients")
    groups = gradient_group_report(model)
    if not all(
        groups[name]["finite"] and groups[name]["nonzero"]
        for name in ("base", "gate", "b2_gate", "b3_gate", "b4_gate")
    ):
        raise SystemExit(f"required gradient group is zero: {groups}")
    b1_gate_gradient = model.g_rec.grad.detach().float().item()
    b2_gate_gradient = model.g_rec_b2.grad.detach().float().item()
    b3_gate_gradient = model.g_rec_b3.grad.detach().float().item()
    b4_gate_gradient = model.g_rec_b4.grad.detach().float().item()
    if not math.isfinite(b4_gate_gradient) or b4_gate_gradient == 0:
        raise SystemExit("new B4 gate gradient is not finite/nonzero")
    ...
    elapsed = time.monotonic() - start
    runtime.training_state["completed_2d2i_updates"] = update
    runtime.training_state["processed_2d2i_targets"] = update * GLOBAL_TARGETS
    ...
    metrics = {
        ...
        "g_rec_b3_raw": model.g_rec_b3.detach().float().item(),
        "tanh_g_rec_b3": model.recurrent_scale_b3.detach().float().item(),
        "g_rec_b4_raw": model.g_rec_b4.detach().float().item(),
        "tanh_g_rec_b4": model.recurrent_scale_b4.detach().float().item(),
        ...
        "g_rec_b4_gradient_preclip": b4_gate_gradient,
        "gradient_norm_before_clip": grad_norm.detach().float().item(),
        "gradient_groups": groups,
        "b9_memory_rms": final_h9_rms,
        ...
        "b4_recurrent_output_rms": final_b4_recurrent_rms,
...
def write_heartbeat(runtime, metrics):
    ...
    durable_json(
        runtime.output / "HEARTBEAT.json",
        {
            ...
            "g_rec_b4_raw": metrics["g_rec_b4_raw"],
            "tanh_g_rec_b4": metrics["tanh_g_rec_b4"],
...
def save_run_checkpoint(runtime, update, kind):
    ...
    try:
        ...
        raise
    if kind == "scientific" and int(update) == MAX_UPDATES:
        persistent = PERSISTENT_FINAL_CHECKPOINT
        lock_path = Path("/workspace/parallel_2d2_master/locks/checkpoint_persist.lock")
        lock_path.parent.mkdir(parents=True, exist_ok=True)
        persistent.parent.mkdir(parents=True, exist_ok=True)
        if persistent.exists():
            raise SystemExit(f"refusing to overwrite persistent final checkpoint: {persistent}")
        with lock_path.open("a+") as lock_handle:
            fcntl.flock(lock_handle.fileno(), fcntl.LOCK_EX)
            temporary = persistent.with_name(f".{persistent.name}.{os.getpid()}.incomplete")
            try:
                shutil.copyfile(checkpoint_path, temporary)
                with temporary.open("rb") as handle:
                    os.fsync(handle.fileno())
                copied_sha = file_sha256(temporary)
                if copied_sha != verification["sha256"]:
                    raise SystemExit("persistent final checkpoint staging SHA mismatch")
                os.replace(temporary, persistent)
                legacy.d0.fsync_directory(persistent.parent)
                if file_sha256(persistent) != verification["sha256"]:
                    raise SystemExit("persistent final checkpoint post-copy SHA mismatch")
                persistent_verification = {
                    **verification,
                    "checkpoint": str(persistent),
                    "local_staged_checkpoint": str(checkpoint_path),
                    "local_and_persistent_sha_match": True,
                    "persist_lock": str(lock_path),
                }
                durable_text(
                    persistent.with_suffix(persistent.suffix + ".sha256"),
                    f"{verification['sha256']}  {persistent.name}\n",
                )
                durable_json(
                    persistent.with_suffix(persistent.suffix + ".verification.json"),
                    persistent_verification,
                )
            finally:
                if temporary.exists():
        ...
        for local_path in (
            ...
            if local_path.exists():
                local_path.unlink()
        runtime.training_state["last_checkpoint"] = str(persistent)
        verification = persistent_verification
        cleanup_path = runtime.output / "storage_cleanup_manifest.json"
        cleanup = read_json(cleanup_path)
        cleanup.setdefault("cleanup_actions", []).append({
            "phase": "post_persistent_final_verification",
            "path": str(checkpoint_path),
            "reason": "local staged final removed only after SHA-matched persistent copy",
            "scientific_final_preserved_at": str(persistent),
        })
...
def reconcile_uncheckpointed_artifacts(output, completed):
    ...
    output = Path(output)
    audit = {
        "schema": "exp2d2i_resume_reconciliation_v1",
    ...
    replay_path = output / "matched_2d2c_data_replay_audit.json"
    replay = read_json(replay_path)
    comparisons = replay["checkpoint_cursor_comparisons"]
    removed = sorted(
        key for key in comparisons if key.isdigit() and int(key) > int(completed)
    )
    if removed:
        before = file_sha256(replay_path)
        for key in removed:
            del comparisons[key]
        replay["passed_so_far"] = all(
            replay[key]
            for key in (
                "source_next_batch_reproduced",
                "source_stream_reproduced",
        ...
        ) and all(row["exact"] for row in comparisons.values())
        durable_json(replay_path, replay)
        audit["files"][replay_path.name] = {
            "before_sha256": before,
            "after_sha256": file_sha256(replay_path),
            "removed_uncheckpointed_keys": removed,
        }
...
def run_train(args):
    require_git(clean=False)
    dirty = [line for line in git_output("status", "--porcelain").splitlines()
    ...
    if int(args.end_update) not in (FORCED_RESTART_UPDATE, MAX_UPDATES):
        raise SystemExit("2D2I train segments must end at local update 96 or 191")
    runtime = initialize_runtime(args)
    completed = int(runtime.training_state["completed_2d2i_updates"])
    ...
    for update in range(completed + 1, int(args.end_update) + 1):
        ...
        print(
            f"2D2I update={update:03d}/{MAX_UPDATES} "
    ...
    commands.setdefault("training_segments", {})[str(args.end_update)] = segment
    durable_json(runtime.output / "commands_and_runtime.json", commands)
    if int(args.end_update) == FORCED_RESTART_UPDATE:
        print("EXPERIMENT_2D2I_UPDATE_96_RESTART_REQUIRED", flush=True)
    else:
        if args.resume:
            resume_path = Path(args.resume).resolve()
            removed = []
            for candidate in (resume_path,
                              resume_path.with_suffix(resume_path.suffix + ".sha256"),
                              resume_path.with_suffix(resume_path.suffix + ".verification.json")):
                if candidate.exists():
                    removed.append({"path": str(candidate), "bytes": candidate.stat().st_size})
                    candidate.unlink()
            cleanup = read_json(runtime.output / "storage_cleanup_manifest.json")
            cleanup.setdefault("cleanup_actions", []).append({
                "phase": "post_final_persistent_verification",
                "reason": "superseded local update-96 checkpoint removed after final scientific checkpoint persisted",
                "removed": removed,
                "update_96_sha_retained_in_checkpoint_manifest": True,
            })
            durable_json(runtime.output / "storage_cleanup_manifest.json", cleanup)
        durable_json(
            runtime.output / "training_complete.json",
            {
                "completed_2d2i_updates": MAX_UPDATES,
                "processed_2d2i_targets": ADDITIONAL_TARGETS,
                "cumulative_2d2_targets": CUMULATIVE_TARGETS,
                "checkpoint": runtime.training_state["last_checkpoint"],
                "timestamp": time.time(),
            },
        )
        print("EXPERIMENT_2D2I_TRAINING_COMPLETE", flush=True)
    return segment
...
def evaluate_incremental(model, val_path, batches=INCREMENTAL_BATCHES) -> dict:
    ...
    for batch_index in range(batches):
        ...
        print(f"2D2I incremental batch={batch_index + 1:02d}/{batches}", flush=True)
        del x, y, cpu_x, cpu_y
        torch.cuda.empty_cache()
    elapsed = time.monotonic() - start
    controls = {}
...
def parallel_incremental_equivalence(model, val_path, length=64, batch=2):
    if length < 64:
        raise ValueError("2D2I equivalence requires length >=64 to exercise B2 W32 eviction")
...
def load_final_model(args, device):
    ...
    if final_path != PERSISTENT_FINAL_CHECKPOINT:
        raise SystemExit(
            f"final checkpoint must be the SHA-verified persistent copy: "
            f"{PERSISTENT_FINAL_CHECKPOINT}"
        )
    verification_path = final_path.with_suffix(
        final_path.suffix + ".verification.json"
    )
    if not verification_path.is_file():
        raise SystemExit("final persistent checkpoint verification sidecar missing")
    persisted_verification = read_json(verification_path)
    payload = legacy.d0.torch_load(final_path, mmap=False)
    if payload.get("schema") != CHECKPOINT_SCHEMA:
        raise SystemExit("final checkpoint schema mismatch")
    if payload.get("completed_2d2i_updates") != MAX_UPDATES:
    ...
    if payload["next_global_batch_stream_sha256"] != global_batch_stream_hash(
        ...
        raise SystemExit("final checkpoint next stream mismatch")
    audit = {
        "checkpoint": str(final_path),
        "sha256": file_sha256(final_path),
        "bytes": final_path.stat().st_size,
        "completed_2d2i_updates": payload["completed_2d2i_updates"],
        "processed_2d2i_targets": payload["processed_2d2i_targets"],
    ...
    audit["passed"] = (
        ...
        and persisted_verification.get("passed") is True
        and persisted_verification.get("local_and_persistent_sha_match") is True
        and persisted_verification.get("sha256") == audit["sha256"]
...
def classify_result(incremental, stable=True, integrity=True) -> str:
    if not integrity:
        return "EXPERIMENT 2D2I INVALID"
    if not stable:
...
def choose_recommendation(classification, initial_damage) -> str:
    if classification == "EXPERIMENT 2D2I INVALID":
        return "FIX 2D2I INTEGRITY"
...
def make_plots(output, milestones, training, incremental, attention, temporal, performance):
    ...
    def line_plot(filename, series, ylabel):
        ...
        ax.set_xlabel("Additional 2D2I training targets")
    ...
    ax.set(xlabel="Additional 2D2I training targets", ylabel="tanh(g_rec_B1)")
    ax.grid(alpha=0.25)
    ax.legend()
    fig.tight_layout()
    ...
    ax.plot(tx, [row["tanh_g_rec_b2"] for row in training], label="2D2I W32")
    ax.plot(
        milestone_targets,
...
def render_report(summary, audit, questions):
    ...
    lines = [
        f"EXPERIMENT 2D2I PRIMARY CLASSIFICATION:\n{summary['primary_classification']}",
        ...
        "\n# Experiment 2D2I — B2 W32 + Older B11→B2 Recurrent K/V",
        "\n## Result",
...
def build_integrity_checks(
    ...
    checks = {
        ...
        "parameter count 124,475,906": preflight["parameters"]["2d2i_total_parameters"] == TOTAL_PARAMETERS,
        ...
        "191 additional optimizer updates": final_model_audit["completed_2d2i_updates"] == 191,
        "100,139,008 additional targets": final_model_audit["processed_2d2i_targets"] == ADDITIONAL_TARGETS,
...
def run_finalize(args):
    ...
    if training_complete["completed_2d2i_updates"] != MAX_UPDATES:
    ...
    summary = {
        ...
        "pod": {
            ...
            "persistent_volume_id": PERSISTENT_VOLUME_IDENTITY,
    ...
    durable_text(output / "EXPERIMENT_2D2I_FINAL_REPORT.md", report)
    durable_text(
    ...
    if not scientific_integrity or not inventory["passed"]:
        raise SystemExit(
            f"2D2I finalize integrity failed: scientific={scientific_integrity} inventory={inventory['passed']}"
        )
    print("EXPERIMENT_2D2I_FINALIZED_PENDING_GIT_SEAL", flush=True)
    return summary
...
def run_seal_report(args):
    ...
    print("EXPERIMENT_2D2I_REPORT_SEALED", flush=True)
...
def run_attest_seal(args):
    ...
    print("EXPERIMENT_2D2I_SEAL_ATTESTED", flush=True)


def reference_2d2e_manifest() -> dict:
    return {
        "schema": "exp2d2i_frozen_2d2e_reference_v1",
        ...
        "b3_effective_gate": SOURCE_B3_GATE_EFFECTIVE,
...
def semantic_diff_audit(model, source_payload) -> dict:
    ...
    report = {
        "baseline": "final Experiment 2D2E",
        "architecture_changes": [
            {"field": "B4 ordinary local window", "old": 1024, "new": 128},
            {"field": "new recurrent link", "old": None, "new": "B9->B4 lags 128...1023"},
            {"field": "new learnable tensor", "old": None, "new": "scalar g_rec_b4"},
        ],
        "new_learnable_tensors": ["g_rec_b4"],
        "source_state_dict_keys_preserved": [
            key for key in target_names if key != "g_rec_b4"
        ] == source_names,
        "b1_gate_resumed": model.g_rec.detach().float().item() == SOURCE_GATE_RAW,
        "b2_gate_resumed": model.g_rec_b2.detach().float().item() == SOURCE_B2_GATE_RAW,
        "b3_gate_resumed": model.g_rec_b3.detach().float().item() == SOURCE_B3_GATE_RAW,
        "b4_gate_zero": model.g_rec_b4.detach().float().item() == 0.0,
        "memory_efficient_source_storage": True,
    }
    report["passed"] = all(
        report[key]
        for key in ("source_state_dict_keys_preserved", "b1_gate_resumed", "b2_gate_resumed", "b3_gate_resumed", "b4_gate_zero")
    )
    if not report["passed"]:
...
def evaluate_parallel(model, val_path, batches=VALIDATION_BATCHES, combined_controls=False) -> dict:
    """Evaluate B4 recurrence and four-link marginal controls on fixed rows."""
    model.eval()
    device = next(model.parameters()).device
    loader = legacy.d1.ExplicitShardLoader([val_path], VALIDATION_B, T)
    names = ["all_real", "b4_off", "b4_shuffled", "b4_full_counterfactual"]
    if combined_controls:
        names.extend((
            "all_shuffled", "b1_off", "b2_off", "b3_off",
            "b1_shuffled", "b2_shuffled", "b3_shuffled",
        ))
    accumulators = {
    ...
    for batch_index in range(int(batches)):
        ...
        for name in names:
            kwargs = {}
            if name == "b4_off": kwargs["b4_gate_override"] = 0.0
            elif name == "b4_shuffled": kwargs["b4_recurrent_permutation"] = permutation
            elif name == "b4_full_counterfactual": kwargs["b4_full_counterfactual"] = True
            elif name == "all_shuffled":
                kwargs.update(b1_recurrent_permutation=permutation,
                              b2_recurrent_permutation=permutation,
                              b3_recurrent_permutation=permutation,
                              b4_recurrent_permutation=permutation)
            elif name == "b1_off": kwargs["b1_gate_override"] = 0.0
            elif name == "b2_off": kwargs["b2_gate_override"] = 0.0
            elif name == "b3_off": kwargs["b3_gate_override"] = 0.0
            elif name == "b1_shuffled": kwargs["b1_recurrent_permutation"] = permutation
            elif name == "b2_shuffled": kwargs["b2_recurrent_permutation"] = permutation
            elif name == "b3_shuffled": kwargs["b3_recurrent_permutation"] = permutation
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
    ...
    real, off = finished["all_real"], finished["b4_off"]
    shuffled, full = finished["b4_shuffled"], finished["b4_full_counterfactual"]
    result = {
        "controls": finished,
        "b4_recurrent_gain": off["validation_loss"] - real["validation_loss"],
        "b4_sequence_gap": shuffled["validation_loss"] - real["validation_loss"],
        "b4_w128_compression_gap_vs_full": real["validation_loss"] - full["validation_loss"],
        "all_real_vs_b4_off": paired_stats(real["per_batch_losses"], off["per_batch_losses"]),
        "all_real_vs_b4_shuffled": paired_stats(real["per_batch_losses"], shuffled["per_batch_losses"]),
        "all_real_vs_b4_full": paired_stats(real["per_batch_losses"], full["per_batch_losses"]),
        "g_rec_b1_raw": model.g_rec.detach().float().item(),
        "tanh_g_rec_b1": model.recurrent_scale_b1.detach().float().item(),
        "g_rec_b2_raw": model.g_rec_b2.detach().float().item(),
        "tanh_g_rec_b2": model.recurrent_scale_b2.detach().float().item(),
    ...
    if combined_controls:
        result["combined_system"] = {
            ...
            "b1_marginal_gain": finished["b1_off"]["validation_loss"] - real["validation_loss"],
            "b2_marginal_gain": finished["b2_off"]["validation_loss"] - real["validation_loss"],
            "b3_marginal_gain": finished["b3_off"]["validation_loss"] - real["validation_loss"],
            "b4_marginal_gain": off["validation_loss"] - real["validation_loss"],
            "b1_shuffled_gap": finished["b1_shuffled"]["validation_loss"] - real["validation_loss"],
            "b2_shuffled_gap": finished["b2_shuffled"]["validation_loss"] - real["validation_loss"],
            "b3_shuffled_gap": finished["b3_shuffled"]["validation_loss"] - real["validation_loss"],
        }
    return result
...
def attention_diagnostics(model, val_path, link, batch_size=2) -> dict:
    ...
    if link in {"b2", "b3", "b4"} and (local_weights is None or local_valid is None):
        raise SystemExit(f"{link} local diagnostic weights unavailable")
    query = torch.arange(T, device=device).view(T, 1)
    source = torch.arange(T, device=device).view(1, T)
    lag = query - source
    recurrent_bins = {"b1": B1_LAG_BINS, "b2": B2_RECURRENT_LAG_BINS,
                      "b3": B3_RECURRENT_LAG_BINS, "b4": B4_RECURRENT_LAG_BINS}[link]
    local_bins_spec = {"b1": (("0-1", 0, 1),), "b2": B2_LOCAL_LAG_BINS,
                       "b3": B3_LOCAL_LAG_BINS, "b4": B4_LOCAL_LAG_BINS}[link]
...
def temporal_gradient_by_lag(model, val_path, link, precision="bf16", gate_override=None, **legacy_kwargs) -> dict:
    ...
    with torch.enable_grad(), context:
        ...
        mapping = {"b1": ("h12", "b1_recurrent_source", "b1_gate_override"),
                   "b2": ("h11", "b2_recurrent_source", "b2_gate_override"),
                   "b3": ("h10", "b3_recurrent_source", "b3_gate_override"),
                   "b4": ("h9", "b4_recurrent_source", "b4_gate_override")}
        ...
        kwargs = {"b1_recurrent_source": first["h12"].detach(),
                  ...
                  "b4_recurrent_source": first["h9"].detach(),
    ...
    minimum = {"b1": 2, "b2": 32, "b3": 64, "b4": 128}[link]
    lag_bins = {"b1": B1_LAG_BINS, "b2": B2_RECURRENT_LAG_BINS,
                "b3": B3_RECURRENT_LAG_BINS, "b4": B4_RECURRENT_LAG_BINS}[link]
    lags = T - 1 - torch.arange(T, device=device)
    bins = {}
    for name, first_lag, last_lag in lag_bins:
    ...
    latest = T - 1 - minimum
    gate = {"b1": model.g_rec, "b2": model.g_rec_b2, "b3": model.g_rec_b3,
            "b4": model.g_rec_b4}[link]
    effective = {"b1": model.recurrent_scale_b1, "b2": model.recurrent_scale_b2,
                 "b3": model.recurrent_scale_b3, "b4": model.recurrent_scale_b4}[link]
    report = {"link": {"b1": "B12->B1", "b2": "B11->B2", "b3": "B10->B3", "b4": "B9->B4"}[link],
...
def attached_writer_gradient_check(model, val_path) -> dict:
    report = {}
    for link, source, latest in (("b1", "h12", 1021), ("b2", "h11", 991),
                                 ("b3", "h10", 959), ("b4", "h9", 895)):
        row = temporal_gradient_by_lag(model, val_path, link, gate_override=(0.05 if link == "b4" else None))
        report[{"b1": "b12_to_b1", "b2": "b11_to_b2", "b3": "b10_to_b3", "b4": "b9_to_b4"}[link]] = {
...
def kernel_preflight(model, short_tokens, short_targets) -> dict:
    ...
    masks = {"b1": model.recurrent_mask(length, length, device),
             "b2": model.b2_recurrent_mask(length, length, device),
             "b3": model.b3_recurrent_mask(length, length, device),
             "b4": model.b4_recurrent_mask(length, length, device)}
    local = {"b1": model.local_mask(length, device), "b2": model.b2_local_mask(length, device),
             "b3": model.b3_local_mask(length, device),
             "b4": model.b4_local_mask(length, device)}
    minima = {"b1": 2, "b2": 32, "b3": 64, "b4": 128}
    checks = {}
    for name in masks:
    ...
    full_b4 = model.b4_recurrent_mask(T, T, device)
    counts = {str(position): int(full_b4[position].sum()) for position in (0, 127, 128, 129, 1023)}
    checks["b4_boundary_counts"] = counts == {"0": 0, "127": 0, "128": 1, "129": 2, "1023": 896}
    values = torch.randn(short_tokens.size(0), length, N_EMBD, device=device)
    banks = (model.build_recurrent_bank(values), model.build_recurrent_bank_b2(values),
             model.build_recurrent_bank_b3(values), model.build_recurrent_bank_b4(values))
    checks["rank_three_unrepeated_banks"] = all(bank.values.ndim == 3 and bank.values.data_ptr() == values.data_ptr() for bank in banks)
    calls = {"b1": 0, "b2": 0, "b3": 0, "b4": 0}
    ...
    try:
        with torch.no_grad():
            active = model.forward_pass(short_tokens, targets=short_targets,
                b1_recurrent_source=values, b2_recurrent_source=values,
                b3_recurrent_source=values, b4_recurrent_source=values,
                b4_gate_override=0.05, return_diagnostics=True)
    finally:
        for hook in hooks: hook.remove()
    checks["single_c_proj_each"] = calls == {"b1": 1, "b2": 1, "b3": 1, "b4": 1}
    with torch.no_grad():
        first = model.forward_pass(short_tokens)
        absent = model.forward_pass(short_tokens, b1_recurrent_source=first["h12"],
                                    b2_recurrent_source=first["h11"], b3_recurrent_source=first["h10"],
                                    b4_gate_override=0.0)["logits"]
        zero = model.forward_pass(short_tokens, b1_recurrent_source=first["h12"],
                                  b2_recurrent_source=first["h11"], b3_recurrent_source=first["h10"],
                                  b4_recurrent_source=first["h9"], b4_gate_override=0.0)["logits"]
    checks["b4_gate_zero_identity"] = torch.equal(absent, zero)
    changed = short_tokens.clone(); changed[:, -1] = (changed[:, -1] + 17) % model.config.vocab_size
    with torch.no_grad():
        base = model.forward_multi_pass(short_tokens, num_passes=2, b4_gate_override=0.05)["logits"]
        altered = model.forward_multi_pass(changed, num_passes=2, b4_gate_override=0.05)["logits"]
    ...
    if short_tokens.size(0) > 1:
        ...
        with torch.no_grad(): isolated = model.forward_multi_pass(changed, num_passes=2, b4_gate_override=0.05)["logits"]
        checks["row_isolation"] = torch.equal(base[1:], isolated[1:])
    return {"checks": checks, "reports": {"b4_boundary_counts": counts, "c_proj_calls": calls}, "passed": all(checks.values())}


def frozen_2d2e_regression(model, source_payload, val_path) -> dict:
    old = source_core.RecurrentKVGPT(model.base)
    old.g_rec = model.g_rec; old.g_rec_b2 = model.g_rec_b2; old.g_rec_b3 = model.g_rec_b3
    observed = source_driver.evaluate_parallel(old, val_path, combined_controls=True)
    loss = observed["controls"]["all_real"]["validation_loss"]
    checks = {"parallel_loss_exact": abs(loss - 3.088601010388447) <= 5e-7,
              ...
              "b2_gate": model.g_rec_b2.detach().float().item() == SOURCE_B2_GATE_RAW,
              "b3_gate": model.g_rec_b3.detach().float().item() == SOURCE_B3_GATE_RAW}
    report = {"expected_all_real_loss": 3.088601010388447, "observed_all_real_loss": loss,
              "full": observed, "checks": checks, "passed": all(checks.values())}
    if not report["passed"]: raise SystemExit(f"frozen 2D2E regression failed: {checks}")
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                for _ in range(3):
                    current = model.forward_pass(x, targets=y, b1_recurrent_source=h12,
                                                 b2_recurrent_source=h11, b3_recurrent_source=h10,
                                                 b4_recurrent_source=h9,
                                                 activation_checkpointing=True)
                    rows.append(current); h9, h10, h11, h12 = current["h9"], current["h10"], current["h11"], current["h12"]
...
def run_preflight(args):
    ...
    source_manifest = {**source_audit, "frozen_2d2e_tag": FROZEN_TAG,
                       "frozen_2d2e_commit": FROZEN_COMMIT,
                       "frozen_tag_exact": git_output("rev-parse", FROZEN_TAG + "^{commit}") == FROZEN_COMMIT,
                       "canonical_validation_sha256": CANONICAL_VALIDATION_SHA256}
    reference = reference_2d2e_manifest()
    durable_json(output / "source_manifest.json", source_manifest)
    durable_json(output / "parameter_manifest.json", parameters)
    durable_json(output / "architecture_manifest.json", architecture)
    durable_json(output / "semantic_diff_audit.json", semantic)
    durable_json(output / "2d2e_reference_manifest.json", reference)

    short_loader = legacy.d1.ExplicitShardLoader([val_path], 2, 136)
    short_x, short_y = short_loader.next_batch()
    kernel = kernel_preflight(model, short_x.to(device), short_y.to(device))
    if not kernel["passed"]: raise SystemExit(f"kernel preflight failed: {kernel['checks']}")
    regression = frozen_2d2e_regression(model, source_payload, val_path)
    ...
    off_loss = zero["controls"]["b4_off"]["validation_loss"]
    initial = {"source_2d2e_loss": source_loss,
               "b4_w128_off_loss": off_loss,
               "b4_w128_real_zero_gate_loss": zero["controls"]["all_real"]["validation_loss"],
               "b4_w128_shuffled_zero_gate_loss": zero["controls"]["b4_shuffled"]["validation_loss"],
               "initial_B4_W128_damage": off_loss - source_loss,
               ...
               "gate_zero_shuffled_identity": zero["controls"]["b4_shuffled"]["validation_loss"] == off_loss}
    initial["passed"] = initial["gate_zero_real_identity"] and initial["gate_zero_shuffled_identity"]
    if not initial["passed"]: raise SystemExit(f"initial B4 gate-zero identity failed: {initial}")

    temporal = {
        "b1": temporal_gradient_by_lag(model, val_path, "b1"),
        "b2": temporal_gradient_by_lag(model, val_path, "b2"),
        "b3": temporal_gradient_by_lag(model, val_path, "b3"),
        "b4_probe": temporal_gradient_by_lag(model, val_path, "b4", gate_override=0.05),
    }
    ...
    attention = {link: attention_diagnostics(model, val_path, link) for link in ("b1", "b2", "b3", "b4")}
    ...
    checks = {
        ...
        "b4_probe_long_gradient": temporal["b4_probe"]["long_lag_writer_gradient_present"],
    ...
    preflight = {"experiment": EXPERIMENT, "protocol": PROTOCOL, "timestamp": time.time(),
                 ...
                 "initial_b4_w128_compression": initial, "temporal_gradient_preflight": temporal,
    ...
    durable_json(output / "paired_controls.json", {"0": {"all_real_vs_b4_off": zero["all_real_vs_b4_off"], "all_real_vs_b4_shuffled": zero["all_real_vs_b4_shuffled"]}})
    durable_json(output / "gate_diagnostics.json", {"0": {key: zero[key] for key in ("g_rec_b1_raw", "tanh_g_rec_b1", "g_rec_b2_raw", "tanh_g_rec_b2", "g_rec_b3_raw", "tanh_g_rec_b3", "g_rec_b4_raw", "tanh_g_rec_b4")}})
    durable_json(output / "b1_attention_diagnostics.json", {"0": attention["b1"]})
    durable_json(output / "b2_attention_diagnostics.json", {"0": attention["b2"]})
    ...
    durable_json(output / "b4_local_attention_lag_bins.json", {"0": attention["b4"]["local_lag_bins"]})
    durable_json(output / "b4_recurrent_attention_lag_bins.json", {"0": attention["b4"]["lag_bins"]})
    durable_json(output / "b4_attention_head_distance.json", {"0": {"heads": attention["b4"]["heads"], "aggregate": attention["b4"]["aggregate"], "head_mean_lag_range": attention["b4"]["head_mean_lag_range"]}})
    for name, row in (("b12_to_b1_temporal_gradient.json", temporal["b1"]),
                      ("b11_to_b2_temporal_gradient.json", temporal["b2"]),
                      ("b10_to_b3_temporal_gradient.json", temporal["b3"]),
                      ("b9_to_b4_temporal_gradient.json", temporal["b4_probe"])):
    ...
    durable_json(output / "distributed_equivalence.json", {"used": False, "physical_pod_gpu_count": 4, "visible_gpu_count": 1, "assigned_physical_gpu": 3, "reason": "one independent process on assigned A100; no DDP", "passed": True})
    durable_json(output / "storage_cleanup_manifest.json", {
        "schema": "exp2d2i_storage_cleanup_manifest_v1",
        "scientific_or_source_checkpoint_removed": False,
        "historical_artifacts_removed": False,
        "master_logical_used_bytes_at_preflight": mount[
            "master_logical_used_bytes_at_preflight"
        ],
        "master_logical_free_bytes_at_preflight": mount[
            "master_logical_free_bytes_at_preflight"
        ],
        "per_command_recursive_volume_rescan_performed": False,
        "fuse_df_treated_as_volume_quota": False,
        "cleanup_actions": [],
    })
    durable_json(output / "commands_and_runtime.json", {"preflight_command": " ".join(sys.argv), "environment": environment_payload()})
    durable_json(output / "persistent_workspace_audit.json", mount)
    durable_json(output / "runpod_stop_capability.json", stop)
    if not preflight["science_passed"]: raise SystemExit(f"2D2I preflight failed: {checks}")
    print("EXPERIMENT_2D2I_PREFLIGHT_PASS", flush=True)
    return preflight
...
def run_smoke(args):
    ...
    for update in range(1, 4):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            first = model.forward_pass(x, targets=y, activation_checkpointing=True)
            # The writer state is non-leaf; retaining its gradient is required
            # for the protocol's actual attached B9->B4 writer-gradient smoke
            # audit once the initially-zero gate has opened.
            first["h9"].retain_grad()
        ...
        before = {"b1": model.g_rec.detach().float().item(),
                  "b2": model.g_rec_b2.detach().float().item(),
                  "b3": model.g_rec_b3.detach().float().item(),
                  "b4": model.g_rec_b4.detach().float().item()}
        loss.backward(); groups = gradient_group_report(model)
        writer_gradient_rms = (0.0 if first["h9"].grad is None else
            first["h9"].grad.detach().float().square().mean().sqrt().item())
        gradients = {"b1": model.g_rec.grad.detach().float().item(),
                     "b2": model.g_rec_b2.grad.detach().float().item(),
                     "b3": model.g_rec_b3.grad.detach().float().item(),
                     "b4": model.g_rec_b4.grad.detach().float().item()}
        ...
        rows.append({"update": update, "loss": loss.detach().float().item(),
                     ...
                     "b9_writer_gradient_rms": writer_gradient_rms,
                     "gradient_norm": norm.detach().float().item(),
                     "gradients_finite": gradients_finite(model),
                     ...
                     "recurrent_states_finite": all(bool(torch.isfinite(tensor).all()) for tensor in
                         (first["h9"], first["h10"], first["h11"], first["h12"],
                          second["h9"], second["h10"], second["h11"], second["h12"]))})
        del x, y, cpu_x, cpu_y, first, second, loss
        torch.cuda.empty_cache()

    # Exercise the complete result checkpoint path after exactly three
    # disposable optimizer updates.  This is intentionally a full checkpoint
    # (model, optimizer, loader/cursor, RNG, counters and next-batch hashes),
    # followed by the same strict fresh-runtime restore used at update 96.
    smoke_path = (
        Path(args.run_root).resolve()
        / "smoke"
        / "2d2i_disposable_update_0003.pt"
    )
    smoke_state = {
        "completed_2d2i_updates": 3,
        "processed_2d2i_targets": 3 * 2 * T,
    ...
    smoke_metadata = {
        "experiment": EXPERIMENT,
        "protocol": PROTOCOL,
        "kind": "exactly_three_update_disposable_smoke",
        "source_checkpoint_sha256": SOURCE_SHA256,
        "micro_batch_sequences": 2,
        "gradient_accumulation": 1,
        "implementation_fingerprint": preflight["implementation_fingerprint"],
    }
    smoke_verification = save_checkpoint(
        smoke_path,
        model,
        optimizer,
        loader,
    ...
    smoke_reload_checks = {
        "full_checkpoint_strict_reopen": smoke_verification["strict_reopen"][
            "passed"
        ],
        "checkpoint_sha_reverified": reopened_smoke_sha
        == smoke_verification["sha256"],
        "model_optimizer_finite_after_restore": model_finite(model)
        and optimizer_finite(optimizer),
        "loader_and_counters_restored": restored_smoke_state == smoke_state,
        "rng_restored_exactly": smoke_rng_restore["passed"],
        "saved_process_recorded": saved_smoke_pid == os.getpid(),
        "exactly_three_updates_in_checkpoint": restored_smoke_state[
            "completed_2d2i_updates"
        ]
        == 3,
    }
    smoke_reload = {
        **smoke_verification,
        "restore_audit": {
            "saved_process_id": saved_smoke_pid,
            "rng": smoke_rng_restore,
            "checks": smoke_reload_checks,
        },
        "passed": all(smoke_reload_checks.values()),
    }
    smoke_bytes = smoke_path.stat().st_size

    cache_x, _ = loader.clone().next_batch()
    with torch.no_grad():
        cache = model.incremental_logits(cache_x[:, :136].to(device), control="all_real", bank_mode="full")["cache_audit"]
    checks = {"exactly_three_updates": len(rows) == 3,
              "b4_gradient_update1_nonzero": math.isfinite(rows[0]["gate_gradients"]["b4"]) and rows[0]["gate_gradients"]["b4"] != 0,
              "b4_gate_changes_from_zero": rows[0]["gate_raw_before"]["b4"] == 0.0 and rows[0]["gate_raw_after"]["b4"] != 0.0,
              "writer_gradient_after_gate_opens": math.isfinite(rows[1]["b9_writer_gradient_rms"]) and rows[1]["b9_writer_gradient_rms"] > 0.0,
              "checkpoint_reload_valid": smoke_reload["passed"],
              "all_gradient_groups_nonzero": all(all(row["gradient_groups"][name]["finite"] and row["gradient_groups"][name]["nonzero"] for name in ("base", "gate", "b2_gate", "b3_gate", "b4_gate")) for row in rows),
              "finite": all(row["parameters_finite"] and row["optimizer_finite"] and row["recurrent_states_finite"] for row in rows),
              "cache_geometry": cache["passed"] and cache["b1_historical_kv"] == 1 and cache["b2_historical_kv"] == 31 and cache["b3_historical_kv"] == 63 and cache["b4_historical_kv"] == 127,
    ...
    audit = {"experiment": EXPERIMENT, "kind": "exactly three disposable optimizer updates",
             ...
             "checkpoint_reload_audit": smoke_reload,
             "checks": checks, "passed": all(checks.values()),
             "disposition": "Discarded after SHA/reload audit; scientific update 1 reloads exact finalized 2D2E."}
    durable_json(output / "smoke_audit.json", audit)
    manifest = read_json(output / "checkpoint_manifest.json")
    manifest["smoke"]["3"] = {
        **smoke_reload,
    ...
    for candidate in (
        ...
        if candidate.exists():
            removed_smoke_paths.append(
                {"path": str(candidate), "bytes": candidate.stat().st_size}
            )
            candidate.unlink()
    cleanup.setdefault("cleanup_actions", []).append({"phase": "post_smoke_reload",
        "removed": removed_smoke_paths, "checkpoint_bytes": smoke_bytes,
        "reason": "disposable 3-update checkpoint and sidecars removed after successful strict reload",
        "scientific_or_source_checkpoint_removed": False})
    durable_json(output / "storage_cleanup_manifest.json", cleanup)
    commands = read_json(output / "commands_and_runtime.json")
    commands["smoke_command"] = " ".join(sys.argv)
    commands["smoke_checkpoint_reload"] = smoke_reload
    durable_json(output / "commands_and_runtime.json", commands)
    if not audit["passed"]: raise SystemExit(f"2D2I smoke failed: {checks}")
    print("EXPERIMENT_2D2I_SMOKE_PASS", flush=True)
    return audit
...
def milestone_diagnostics(runtime, update, val_path):
    ...
    validation.update(local_update=update, additional_targets=update * GLOBAL_TARGETS,
                      cumulative_2d2_targets=SOURCE_TARGETS + update * GLOBAL_TARGETS)
    attention = {link: attention_diagnostics(runtime.model, val_path, link) for link in ("b1", "b2", "b3", "b4")}
    temporal = {link: temporal_gradient_by_lag(runtime.model, val_path, link) for link in ("b1", "b2", "b3", "b4")}
    ...
    merge_keyed_json(runtime.output / "paired_controls.json", update,
                     {"all_real_vs_b4_off": validation["all_real_vs_b4_off"],
                      "all_real_vs_b4_shuffled": validation["all_real_vs_b4_shuffled"]})
    merge_keyed_json(runtime.output / "gate_diagnostics.json", update,
                     {key: validation[key] for key in ("g_rec_b1_raw", "tanh_g_rec_b1", "g_rec_b2_raw", "tanh_g_rec_b2", "g_rec_b3_raw", "tanh_g_rec_b3", "g_rec_b4_raw", "tanh_g_rec_b4")})
    merge_keyed_json(runtime.output / "b1_attention_diagnostics.json", update, attention["b1"])
    merge_keyed_json(runtime.output / "b2_attention_diagnostics.json", update, attention["b2"])
    ...
    merge_keyed_json(runtime.output / "b4_local_attention_lag_bins.json", update, attention["b4"]["local_lag_bins"])
    merge_keyed_json(runtime.output / "b4_recurrent_attention_lag_bins.json", update, attention["b4"]["lag_bins"])
    merge_keyed_json(runtime.output / "b4_attention_head_distance.json", update,
                     {"heads": attention["b4"]["heads"], "aggregate": attention["b4"]["aggregate"],
                      "head_mean_lag_range": attention["b4"]["head_mean_lag_range"]})
    for name, link in (("b12_to_b1_temporal_gradient.json", "b1"),
                       ("b11_to_b2_temporal_gradient.json", "b2"),
                       ("b10_to_b3_temporal_gradient.json", "b3"),
                       ("b9_to_b4_temporal_gradient.json", "b4")):
...
def reconcile_uncheckpointed_artifacts(output, completed):
    ...
    keyed = ("milestone_validation.json", "paired_controls.json", "gate_diagnostics.json",
             "b1_attention_diagnostics.json", "b2_attention_diagnostics.json",
             ...
             "b4_local_attention_lag_bins.json", "b4_recurrent_attention_lag_bins.json",
             "b4_attention_head_distance.json", "b11_to_b2_temporal_gradient.json",
             "b10_to_b3_temporal_gradient.json", "b9_to_b4_temporal_gradient.json")
    for name in keyed:
        path = output / name
...
def _incremental_control(model, x, y, name, derangement=None):
    batch, length = x.shape
    state = model.init_incremental_state(batch, device=x.device, b4_full_cache=name == "b4_full_counterfactual")
    ...
    ring_max = {"h9": 0, "h10": 0, "h11": 0, "h12": 0}
    rms = {"b1": [], "b2": [], "b3": [], "b4": []}
    for position in range(length):
        logits, state, diagnostics = model.incremental_step(
            x[:, position], state, control=name,
            recurrent_permutation=derangement if name in {"b4_shuffled", "all_shuffled"} else None,
...
@torch.no_grad()
def evaluate_incremental(model, val_path, batches=INCREMENTAL_BATCHES) -> dict:
    if int(batches) != 4: raise ValueError("2D2I primary incremental evaluation requires exactly four B64 batches")
    model.eval(); device = next(model.parameters()).device
    loader = legacy.d1.ExplicitShardLoader([val_path], VALIDATION_B, T)
    names = ("all_real", "b4_off", "b4_shuffled", "b4_full_counterfactual")
    ...
    real, off, shuffled = controls["all_real"], controls["b4_off"], controls["b4_shuffled"]
    first_real, first_off, first_shuffled = (controls[name]["first_131072_validation_loss"] for name in ("all_real", "b4_off", "b4_shuffled"))
    result = {"controls": controls,
              "true_b4_recurrent_gain": off["validation_loss"] - real["validation_loss"],
              "true_b4_sequence_gap": shuffled["validation_loss"] - real["validation_loss"],
              "first_131072": {"true_b4_recurrent_gain": first_off - first_real,
                                 "true_b4_sequence_gap": first_shuffled - first_real,
                                 "controls": {name: controls[name]["first_131072_validation_loss"] for name in names}},
              "all_real_vs_b4_off_batches": paired_stats(real["per_batch_losses"], off["per_batch_losses"]),
              "all_real_vs_b4_shuffled_batches": paired_stats(real["per_batch_losses"], shuffled["per_batch_losses"]),
              "all_real_vs_b4_off_sequences": paired_stats(real["per_sequence_losses"], off["per_sequence_losses"]),
              "all_real_vs_b4_shuffled_sequences": paired_stats(real["per_sequence_losses"], shuffled["per_sequence_losses"]),
              "g_rec_b1_raw": model.g_rec.detach().float().item(), "tanh_g_rec_b1": model.recurrent_scale_b1.detach().float().item(),
              "g_rec_b2_raw": model.g_rec_b2.detach().float().item(), "tanh_g_rec_b2": model.recurrent_scale_b2.detach().float().item(),
              "g_rec_b3_raw": model.g_rec_b3.detach().float().item(), "tanh_g_rec_b3": model.recurrent_scale_b3.detach().float().item(),
              "g_rec_b4_raw": model.g_rec_b4.detach().float().item(), "tanh_g_rec_b4": model.recurrent_scale_b4.detach().float().item(),
...
@torch.no_grad()
def parallel_incremental_equivalence(model, val_path, length=136, batch=2):
    if length < 129: raise ValueError("equivalence must exercise B4 W128 eviction and active recurrence")
    ...
    for label in ("fp32", "bf16"):
        ...
        reports[label] = {"plain_kernel_max_abs": delta.max().item(), "plain_kernel_mean_abs": delta.mean().item(),
                          ...
                          "b4_w128_eviction_exercised": True}
    torch.set_float32_matmul_precision(original[0]); torch.backends.cuda.matmul.allow_tf32 = original[1]; torch.backends.cudnn.allow_tf32 = original[2]
...
def memory_accounting(incremental=None) -> dict:
    ...
    def one(batch):
        ...
        b4 = batch * 127 * N_EMBD * 2 * element_bytes
        rings = batch * 4 * 1023 * N_EMBD * element_bytes
        upper = batch * 8 * 1023 * N_EMBD * 2 * element_bytes
        total = b1 + b2 + b3 + b4 + rings + upper
        standard = batch * 12 * 1023 * N_EMBD * 2 * element_bytes
        final_2d2e = batch * (1 * N_EMBD * 2 + 31 * N_EMBD * 2 + 63 * N_EMBD * 2 + 3 * 1023 * N_EMBD + 9 * 1023 * N_EMBD * 2) * element_bytes
        return {"batch_size": batch, "b1_local_kv_bytes": b1, "b2_local_kv_bytes": b2,
                "b3_local_kv_bytes": b3, "b4_local_kv_bytes": b4,
                "four_recurrent_raw_state_rings_bytes": rings,
                "b5_b12_ordinary_kv_bytes": upper, "total_experimental_inference_state_bytes": total,
                "final_2d2e_inference_state_bytes": final_2d2e, "standard_gpt2_w1024_kv_bytes": standard,
                "delta_bytes_vs_final_2d2e": total - final_2d2e,
                "delta_bytes_vs_standard_gpt2": total - standard,
    ...
    if incremental is not None:
        ...
        rings = sum(cache[f"{name}_ring_physical_storage"]["actual_bytes"] for name in ("h9", "h10", "h11", "h12"))
    ...
    return {"deployment_accounting_dtype": "BF16", "bytes_per_element": element_bytes,
            ...
            "state_limits": {"B1 ordinary historical KV": 1, "B2 ordinary historical KV": 31,
                             "B3 ordinary historical KV": 63, "B4 ordinary historical KV": 127,
                             "B9/B10/B11/B12 raw recurrent states each": 1023,
                             "B5-B12 ordinary historical KV each": 1023}}


def classify_result(incremental, stable=True, integrity=True) -> str:
    if not integrity: return "EXPERIMENT 2D2I INVALID"
    if not stable: return "B9→B4 W128 RECURRENT LINK IS UNSTABLE"
    off = incremental["all_real_vs_b4_off_sequences"]; shuffled = incremental["all_real_vs_b4_shuffled_sequences"]
    gain, gap = incremental["true_b4_recurrent_gain"], incremental["true_b4_sequence_gap"]
    ...
    if positive and gain >= 0.001 and off["wins"] >= 166 and shuffled["wins"] >= 166:
        return "B9→B4 W128 RECURRENT LINK STRONGLY ESTABLISHES UTILITY"
    if positive: return "B9→B4 W128 RECURRENT LINK ESTABLISHES POSITIVE UTILITY"
    if gain < 0 and off["losses"] >= 129:
        return "B9→B4 W128 RECURRENT LINK IS HARMFUL"
    if gap > 0: return "B9→B4 W128 RECURRENCE IS SEQUENCE-SPECIFIC BUT DOES NOT ESTABLISH UTILITY"
    balanced = 112 <= off["wins"] <= 144 and 112 <= shuffled["wins"] <= 144
    if abs(gain) < 1e-4 and abs(gap) < 1e-4 and balanced:
        return "B9→B4 W128 RECURRENT LINK REMAINS NEAR ZERO"
    return "B9→B4 W128 RECURRENT LINK DOES NOT ESTABLISH POSITIVE UTILITY"


def choose_recommendation(classification, initial_damage):
    if classification == "EXPERIMENT 2D2I INVALID": return "FIX 2D2I INTEGRITY"
    if classification == "B9→B4 W128 RECURRENT LINK IS UNSTABLE": return "STABILIZE B9→B4 W128 RECURRENCE"
    if classification == "B9→B4 W128 RECURRENT LINK IS HARMFUL":
        return "REMOVE THE B9→B4 LINK AND RETAIN THE BEST MASTER-MATRIX ARCHITECTURE"
    if classification in {"B9→B4 W128 RECURRENT LINK ESTABLISHES POSITIVE UTILITY", "B9→B4 W128 RECURRENT LINK STRONGLY ESTABLISHES UTILITY"}:
        return "COMPARE FOUR-LINK UTILITY AGAINST THE MASTER MATRIX BEFORE ANY FIFTH LINK"
    if classification == "B9→B4 W128 RECURRENCE IS SEQUENCE-SPECIFIC BUT DOES NOT ESTABLISH UTILITY":
        return "IMPROVE THE B4 RECURRENT READOUT WITHOUT ADDING A FIFTH LINK"
    if classification == "B9→B4 W128 RECURRENT LINK REMAINS NEAR ZERO":
        return "STOP BLIND MIRRORED-LINK EXTENSION AND TEST A READOUT ABLATION"
    if initial_damage > 0.10:
        return "INCREASE B4 LOCAL WINDOW BEFORE ANY NEW RECURRENT LINK"
    return "IMPROVE THE B4 RECURRENT READOUT WITHOUT ADDING A FIFTH LINK"
...
def self_composition_diagnostic(model, val_path, passes=8, batch_size=2):
    ...
    x, y = cpu_x.to(device), cpu_y.to(device); h9 = h10 = h11 = h12 = None; rows = []
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
...
def _plot_final(output, milestones, training, incremental, attention, temporal, memory, performance, parallel):
    ...
    save(1, lambda ax: (ax.plot(targets, [milestones[str(k)]["controls"]["all_real"]["validation_loss"] for k in updates], marker="o", label="ALL_REAL"), ax.plot(targets, [milestones[str(k)]["controls"]["b4_off"]["validation_loss"] for k in updates], marker="o", label="B4_OFF"), ax.set(xlabel="Additional targets", ylabel="Validation CE"), ax.legend()))
    save(2, lambda ax: (ax.plot(targets, [milestones[str(k)]["b4_recurrent_gain"] for k in updates], marker="o"), ax.axhline(0, color="black", linewidth=.8), ax.set(xlabel="Additional targets", ylabel="B4 recurrent gain")))
    save(3, lambda ax: (ax.plot(targets, [milestones[str(k)]["b4_sequence_gap"] for k in updates], marker="o"), ax.axhline(0, color="black", linewidth=.8), ax.set(xlabel="Additional targets", ylabel="B4 sequence gap")))
    save(4, lambda ax: ([ax.plot([0] + [row["additional_targets"] for row in training], [source] + [row[field] for row in training], label=label) for field, source, label in (("tanh_g_rec_b1", SOURCE_GATE_EFFECTIVE, "B1"), ("tanh_g_rec_b2", SOURCE_B2_GATE_EFFECTIVE, "B2"), ("tanh_g_rec_b3", SOURCE_B3_GATE_EFFECTIVE, "B3"), ("tanh_g_rec_b4", 0.0, "B4"))], ax.set(xlabel="Additional targets", ylabel="Effective gate"), ax.legend()))
    local = attention["b4"]["local_lag_bins"]
    save(5, lambda ax: (ax.bar(list(local), [row["attention_mass"] for row in local.values()]), ax.set(ylabel="Attention mass", title="B4 local attention"), ax.tick_params(axis="x", rotation=30)))
    recurrent = attention["b4"]["lag_bins"]
    save(6, lambda ax: (ax.bar(list(recurrent), [row["attention_mass"] for row in recurrent.values()]), ax.set(ylabel="Attention mass", title="B4 recurrent attention"), ax.tick_params(axis="x", rotation=30)))
    save(7, lambda ax: (ax.bar(list(recurrent), [row["normalized_mass_per_available_token"] for row in recurrent.values()]), ax.set(ylabel="Mass / available token", title="Normalized B4 recurrent density"), ax.tick_params(axis="x", rotation=30)))
    heads = attention["b4"]["heads"]
    save(8, lambda ax: (ax.bar([int(k) for k in heads], [v["mean_attended_recurrent_lag"] for v in heads.values()]), ax.set(xlabel="B4 head", ylabel="Mean recurrent lag")))
    b4_temporal = temporal["b4"]["bins"]
    save(9, lambda ax: (ax.bar(list(b4_temporal), [v["mean_gradient_rms"] for v in b4_temporal.values()]), ax.set_yscale("log"), ax.set(ylabel="Writer-gradient RMS", title="B9→B4 by lag"), ax.tick_params(axis="x", rotation=30)))
    ...
    save(11, lambda ax: (ax.bar(["Parallel", "True incremental"], [parallel["b4_recurrent_gain"], incremental["true_b4_recurrent_gain"]]), ax.axhline(0, color="black", linewidth=.8), ax.set(ylabel="B4 recurrent gain")))
    marginals = parallel["combined_system"]
    save(12, lambda ax: (ax.bar(["B1", "B2", "B3", "B4"], [marginals["b1_marginal_gain"], marginals["b2_marginal_gain"], marginals["b3_marginal_gain"], marginals["b4_marginal_gain"]]), ax.axhline(0, color="black", linewidth=.8), ax.set(ylabel="Marginal recurrent gain")))
    unit = N_EMBD * 2
    ...
    comparisons = {
        ...
        "2D2E": memory["B1"]["final_2d2e_inference_state_bytes"],
        "2D2I": memory["B1"]["total_experimental_inference_state_bytes"],
    }
...
def _scientific_questions(initial, training, milestones, attention, temporal, parallel, incremental, memory, classification, recommendation):
    first_nonzero = next((row for row in training if row["g_rec_b4_raw"] != 0.0), None)
    values = [0.0] + [row["tanh_g_rec_b4"] for row in training]
    m = lambda update: milestones[str(update)]["b4_recurrent_gain"]
    heads = [row["mean_attended_recurrent_lag"] for row in attention["b4"]["heads"].values()]
    b4bins = attention["b4"]["lag_bins"]
    q = {
        "Q1": initial["initial_B4_W128_damage"],
        "Q2": initial["damage_relative_to_source_loss"],
        "Q3": training[0]["g_rec_b4_gradient_preclip"] != 0.0,
        ...
        "Q5": None if first_nonzero is None else ("positive" if first_nonzero["g_rec_b4_raw"] > 0 else "negative"),
        "Q6": max(values), "Q7": min(values), "Q8": training[-1]["tanh_g_rec_b4"],
        "Q9": training[-1]["tanh_g_rec_b3"], "Q10": training[-1]["tanh_g_rec_b2"],
        "Q11": temporal["b4"]["nonzero"],
        "Q12": temporal["b4"]["bins"]["128-255"]["fraction_nonzero_positions"] > 0,
        "Q13": temporal["b4"]["bins"]["256-511"]["fraction_nonzero_positions"] > 0,
        "Q14": temporal["b4"]["bins"]["512-1023"]["fraction_nonzero_positions"] > 0,
        "Q15": m(20), "Q16": m(48), "Q17": m(96), "Q18": m(143), "Q19": m(191),
        "Q20": parallel["b4_sequence_gap"], "Q21": parallel["all_real_vs_b4_off"],
        "Q22": parallel["all_real_vs_b4_shuffled"],
        "Q23": b4bins["128-255"]["attention_mass"], "Q24": b4bins["256-511"]["attention_mass"],
        "Q25": b4bins["512-1023"]["attention_mass"], "Q26": attention["b4"]["aggregate"]["effective_positions"] if "effective_positions" in attention["b4"]["aggregate"] else None,
        "Q27": attention["b4"]["aggregate"],
        "Q28": {"head_mean_lag_range": max(heads) - min(heads), "per_head_mean_lags": heads},
        "Q29": parallel["combined_system"],
        "Q30": {"source": SOURCE_B3_GATE_EFFECTIVE, "final": training[-1]["tanh_g_rec_b3"], "change": training[-1]["tanh_g_rec_b3"] - SOURCE_B3_GATE_EFFECTIVE},
        "Q31": training[-1]["tanh_g_rec_b1"],
        "Q32": incremental["true_b4_recurrent_gain"], "Q33": incremental["true_b4_sequence_gap"],
        "Q34": incremental["all_real_vs_b4_off_sequences"], "Q35": incremental["all_real_vs_b4_shuffled_sequences"],
        "Q36": parallel["combined_system"].get("b4_marginal_gain"),
        "Q37": {"parallel_gain": parallel["b4_recurrent_gain"], "true_incremental_gain": incremental["true_b4_recurrent_gain"], "same_sign": parallel["b4_recurrent_gain"] * incremental["true_b4_recurrent_gain"] > 0},
        "Q38": memory["B1"]["saving_bytes_vs_standard_gpt2"],
        "Q39": classification in {"B9→B4 W128 RECURRENT LINK ESTABLISHES POSITIVE UTILITY", "B9→B4 W128 RECURRENT LINK STRONGLY ESTABLISHES UTILITY"},
        "Q40": recommendation,
    }
    return {key: {"question": f"2D2I audit question {key[1:]}", "answer": value}
            for key, value in q.items()}


def render_report(summary, audit, questions):
    inc = summary["incremental"]; initial = summary["initial_b4_w128_compression"]
    lines = [
        "EXPERIMENT 2D2I PRIMARY CLASSIFICATION:", summary["primary_classification"], "",
        "INITIAL B4-W128 COMPRESSION DAMAGE:", str(initial["initial_B4_W128_damage"]), "",
        "FINAL TRUE-SELF B9→B4 W128 RECURRENT GAIN:", str(inc["true_b4_recurrent_gain"]), "",
        "FINAL TRUE-SELF B9→B4 W128 SEQUENCE GAP:", str(inc["true_b4_sequence_gap"]), "",
        "# Experiment 2D2I Final Report", "",
        ...
        f"- Parameters: {TOTAL_PARAMETERS:,} (exactly one new scalar `g_rec_b4`)",
        "- Geometry: B1 W2 + B12 lags 2…1023; B2 W32 + B11 lags 32…1023; B3 W64 + B10 lags 64…1023; B4 W128 + B9 lags 128…1023; B5–B12 W1024",
        ...
        "", "## Milestone B4 metrics", "",
        "| Update | Targets | Gain | Sequence gap | B1 gate | B2 gate | B3 gate | B4 gate |", "|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for key in sorted(summary["validation_trajectory"], key=int):
        row = summary["validation_trajectory"][key]
        lines.append(f"| {key} | {row['additional_targets']} | {row['b4_recurrent_gain']:.12g} | {row['b4_sequence_gap']:.12g} | {row['tanh_g_rec_b1']:.12g} | {row['tanh_g_rec_b2']:.12g} | {row['tanh_g_rec_b3']:.12g} | {row['tanh_g_rec_b4']:.12g} |")
    ...
    lines += ["# EXPERIMENT 2D2I COMPLETE", ""]
    return "\n".join(lines)
...
def run_finalize(args):
    ...
    attention = {link: attention_diagnostics(model, validation_path(args.data_root), link) for link in ("b1", "b2", "b3", "b4")}
    temporal = {link: temporal_gradient_by_lag(model, validation_path(args.data_root), link) for link in ("b1", "b2", "b3", "b4")}
    memory = memory_accounting(incremental)
    walls = [row["wall_seconds"] for row in training]
    ...
    commands["finalize_timestamp"] = time.time()
    durable_json(output / "commands_and_runtime.json", commands)
    ...
    initial = preflight["initial_b4_w128_compression"]
    recommendation = choose_recommendation(classification, initial["initial_B4_W128_damage"])
    ...
    durable_json(output / "b2_attention_diagnostics.json", {**read_json(output / "b2_attention_diagnostics.json"), "final": attention["b2"]})
    ...
    durable_json(output / "b4_local_attention_lag_bins.json", {**read_json(output / "b4_local_attention_lag_bins.json"), "final": attention["b4"]["local_lag_bins"]})
    durable_json(output / "b4_recurrent_attention_lag_bins.json", {**read_json(output / "b4_recurrent_attention_lag_bins.json"), "final": attention["b4"]["lag_bins"]})
    durable_json(output / "b4_attention_head_distance.json", {**read_json(output / "b4_attention_head_distance.json"), "final": {"heads": attention["b4"]["heads"], "aggregate": attention["b4"]["aggregate"], "head_mean_lag_range": attention["b4"]["head_mean_lag_range"]}})
    for filename, link in (("b12_to_b1_temporal_gradient.json", "b1"), ("b11_to_b2_temporal_gradient.json", "b2"), ("b10_to_b3_temporal_gradient.json", "b3"), ("b9_to_b4_temporal_gradient.json", "b4")):
    ...
    checks = {
        "source 2D2E checkpoint SHA exact": file_sha256(args.source_checkpoint) == SOURCE_SHA256,
        "source final tag exact": git_output("rev-parse", FROZEN_TAG + "^{commit}") == FROZEN_COMMIT,
        "parameter count 124,475,908": preflight["parameters"]["2d2i_total_parameters"] == TOTAL_PARAMETERS,
        "exactly one new parameter": preflight["parameters"]["new_parameter_count_vs_2d2e"] == 1,
        "B1 W2 and recurrent lags exact": preflight["kernel_preflight"]["checks"]["b1_mask_exact"],
        "B2 W32 and recurrent lags exact": preflight["kernel_preflight"]["checks"]["b2_mask_exact"],
        "B3 W64 and recurrent lags exact": preflight["kernel_preflight"]["checks"]["b3_mask_exact"],
        "B4 W128 and recurrent lags exact": preflight["kernel_preflight"]["checks"]["b4_mask_exact"],
        "B4 max recurrent width 896": preflight["kernel_preflight"]["reports"]["b4_boundary_counts"]["1023"] == 896,
        "B4 local/recurrent non-overlap": preflight["kernel_preflight"]["checks"]["b4_nonoverlap"],
        "existing projections and one c_proj reused": preflight["kernel_preflight"]["checks"]["single_c_proj_each"],
        "B1/B2/B3 gates resumed; B4 zero initialized": preflight["semantic_diff"]["b1_gate_resumed"] and preflight["semantic_diff"]["b2_gate_resumed"] and preflight["semantic_diff"]["b3_gate_resumed"] and preflight["semantic_diff"]["b4_gate_zero"],
        "all source optimizer state resumed; fresh only B4": preflight["source"]["checks"]["source_optimizer_state_preserved"] and preflight["source"]["checks"]["new_b4_optimizer_state_absent"],
        "data stream continues after 2D2E": preflight["checks"]["loader_continuation"],
        ...
        "persistent final strict load verified": final_model["passed"]
        and Path(final_model["checkpoint"]) == PERSISTENT_FINAL_CHECKPOINT,
    ...
    checks["all pre-report protocol artifacts present"] = all(
        (output / name).is_file() and (output / name).stat().st_size > 0
        for name in REQUIRED_ARTIFACTS
        if name not in pending_report_artifacts
    )
    checks["all required diagnostic plots rendered"] = all(
        (output / name).is_file() and (output / name).stat().st_size > 0
        for name in REQUIRED_PLOTS
    )
    summary = {"experiment": EXPERIMENT, "primary_classification": classification,
               ...
               "initial_b4_w128_compression": initial, "validation_trajectory": milestones,
               ...
               "final_gates": {"b1": final_model["tanh_g_rec_b1"], "b2": final_model["tanh_g_rec_b2"], "b3": final_model["tanh_g_rec_b3"], "b4": final_model["tanh_g_rec_b4"]},
               ...
               "persistent_volume": {"identity": PERSISTENT_VOLUME_IDENTITY, "preserved": True}}
    ...
    durable_text(output / "EXPERIMENT_2D2I_FINAL_REPORT.md", report_text)
    ...
    if not inventory["passed"]:
        missing = [
            name
            for name, row in {
                **inventory["required_artifacts"],
                **inventory["required_plots"],
            }.items()
            if not row["exists_nonempty"]
        ]
        raise SystemExit(f"2D2I required artifact inventory incomplete: {missing}")
    print("EXPERIMENT_2D2I_FINALIZED_PENDING_GIT_SEAL", flush=True)
    return summary
