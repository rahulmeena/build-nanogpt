...
lane_init 1 "2D2G-A then 2D2G-B"
...
A96="$EPHEMERAL/stage_a_scientific_update_0096.pt"
...
log_command 2D2G_PREFLIGHT python scripts/experiment_2d2g.py preflight "${COMMON[@]}" \
  --source-checkpoint "$SOURCE" --data-root "$DATA_ROOT"
log_command 2D2G_A_TRAIN_TO_96 python scripts/experiment_2d2g.py train-a "${COMMON[@]}" \
  --source-checkpoint "$SOURCE" --checkpoint-dir "$EPHEMERAL" \
  --end-update 96 --data-root "$DATA_ROOT"
log_command 2D2G_A_RESUME_TO_191 python scripts/experiment_2d2g.py train-a "${COMMON[@]}" \
  --resume "$A96" --checkpoint-dir "$EPHEMERAL" \
  --end-update 191 --data-root "$DATA_ROOT"
log_command 2D2G_B_SMOKE python scripts/experiment_2d2g.py smoke-b "${COMMON[@]}" \
  --stage-a-checkpoint "$A191" --checkpoint-dir "$SMOKE_EPHEMERAL" \
  --data-root "$DATA_ROOT"
log_command 2D2G_B_TRAIN_TO_96 python scripts/experiment_2d2g.py train-b "${COMMON[@]}" \
  --stage-a-checkpoint "$A191" --checkpoint-dir "$EPHEMERAL" \
  --end-update 96 --data-root "$DATA_ROOT"
log_command 2D2G_B_RESUME_TO_191 python scripts/experiment_2d2g.py train-b "${COMMON[@]}" \
  --resume "$B96" --checkpoint-dir "$EPHEMERAL" \
  --end-update 191 --data-root "$DATA_ROOT"
log_command 2D2G_PERSIST_FINAL python scripts/experiment_2d2g.py persist-final \
  --output-dir "$OUTPUT" \
  --local-checkpoint "$B191" \
  --persistent-dir "$PERSISTENT" \
  --lock-path "$MASTER_ROOT/locks/checkpoint_persist.lock"
log_command 2D2G_FINALIZE python scripts/experiment_2d2g.py finalize "${COMMON[@]}" \
