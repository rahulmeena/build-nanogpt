...
def finalize(archive):
    ...
    with (out/'endpoint_sequences.csv').open('w') as f:
        w=csv.writer(f);w.writerow(['sequence_id','canonical_group','token_sha256','CE_L','CE_H','difference'])
...
Primary paired difference CE(L)−CE(H): **{s['mean']:.10f}**, sequence-bootstrap 95% interval **[{s['lo']:.10f}, {s['hi']:.10f}]**. Positive favors H. Relative PPL difference {s['relative_ppl_percent']:.7f}% with transformed interval {s['relative_ppl_interval_percent']}. Both conditions used one fresh paired 4,096-sequence panel and the same B{l['binding']['batch_size']} true incremental BF16 forward, FP32 token CE and FP64 NLL accumulation. Coverage: 8,192 sequence-condition rows / 8,388,608 final target predictions; five L monitors add 6,553,600. Every sequence has 1,024 scored targets.

Primary flags at reference ΔCE=0.0001: `{json.dumps(s['flags'],sort_keys=True)}`. Failure to establish benefit is not equivalence. The reference is a small-effect convention, not a deployment-value threshold.

Canonical B64 group sensitivity: [{g['lo']:.10f}, {g['hi']:.10f}], flags `{json.dumps(g['flags'],sort_keys=True)}`. Sampling unit changes flags: {a['sampling_unit_changes_flags']}. Both bootstraps used 50,000 FP64 resamples, seeds 20260925/20260926 and linear percentiles. Neither interval estimates independent training-seed variability. Sequence fractions: L wins {a['L_win_fraction']:.6f}, H wins {a['H_win_fraction']:.6f}, ties {a['tie_fraction']:.6f}.
